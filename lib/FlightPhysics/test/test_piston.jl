...
function Modeling.f_step!(harness::Model{<:TestHarness})
    (; fuel_available) = harness.u
    f_step!(harness.thruster, fuel_available)
end

Modeling.f_periodic!(::Unconditional, ::Model{<:TestHarness}) = nothing
...
function test_thruster_response()
    ...
    @testset verbose = true "Thruster Dynamics" begin
        ...
        mdl = TestHarness() |> Model

        sim = Simulation(mdl, t_end = 100)
        ...
        mdl.thruster.engine.u.throttle = 1
        step!(sim, 5, true)
        @test mdl.y.thruster.engine.ω > 2mdl.thruster.engine.ω_idle

        #back to idle, make sure the idle controller kicks back in and idle RPMs
        #stabilize around their target value
        mdl.thruster.engine.u.throttle = 0
        step!(sim, 5, true)
        @test mdl.y.thruster.engine.ω ≈ mdl.thruster.engine.ω_idle atol = 1
        ...
        ω_tmp = mdl.y.thruster.engine.ω
        mdl.thruster.propeller.u[] = 0.2
        step!(sim, 5, true)
        @test mdl.y.thruster.engine.ω < ω_tmp
        ...
        #friction
        @test mdl.y.thruster.engine.ω ≈ 0.0 atol = 1e-10

        #start the engine again to test for allocations
        mdl.u.fuel_available = true
        mdl.thruster.engine.u.start = true
        step!(sim, 5, true)
        mdl.thruster.engine.u.start = false
