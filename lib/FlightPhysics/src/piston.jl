...
@kwdef struct PistonEngineLookup{D, M, PS, PW, PR, SR, SP}
    δ_wot::D
    μ_wot::M
    π_std::PS
    π_wot::PW
...
function PistonEngineLookup(n_stall::Real, n_max::Real)
    ...
    PistonEngineLookup(; δ_wot, μ_wot, π_std, π_wot, π_ratio, sfc_ratio, sfc_pow )
...
struct PistonEngine{L <: PistonEngineLookup} <: AbstractPistonEngine
    P_rated::Float64
    ω_rated::Float64
...
function PistonEngine(;
    ...
    n_max = ω_max / ω_rated
    lookup = PistonEngineLookup(n_stall, n_max)
