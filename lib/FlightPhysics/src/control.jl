...
function GUI.draw(mdl::Model{<:PIVector{N}}, args...) where {N}
    ...
    (; k_p, k_i, k_l, β_p, bound_lo, bound_hi, input, sat_ext,
        ...
        TextFormatted("Proportional Gain = $k_p")
        TextFormatted("Integral Gain = $k_i")
        TextFormatted("Integrator Leak Factor = $k_l")
        TextFormatted("Proportional Input Weighting = $β_p")
        TextFormatted("Lower Output Bound = $bound_lo")
        TextFormatted("Upper Output Bound = $bound_hi")
        TextFormatted("Input = $input")
        TextFormatted("External Saturation Input = $sat_ext")
        TextFormatted("Proportional Path Input = $u_p")
        TextFormatted("Proportional Path Output = $y_p")
        TextFormatted("Integrator State = $x_i")
        TextFormatted("Integral Path Input = $u_i")
        TextFormatted("Integral Path Output = $y_i")
...
function GUI.draw(mdl::Union{Model{<:Integrator}, Model{<:IntegratorVector}}, args...)
    ...
    (; input, sat_ext, bound_lo, bound_hi, x1, output, sat_out, halted) = mdl.y
        ...
        TextFormatted("input = $input")
        TextFormatted("sat_ext = $sat_ext")
        TextFormatted("bound_lo = $bound_lo")
        TextFormatted("bound_hi = $bound_hi")
        TextFormatted("x0 = $x0")
        TextFormatted("sat_out_0 = $sat_out_0")
        TextFormatted("x1 = $x1")
        TextFormatted("output = $output")
        TextFormatted("sat_out = $sat_out")
        TextFormatted("halted = $halted")
...
function GUI.draw(mdl::Union{Model{<:PID}, Model{<:PIDVector}}, args...)
    ...
    TextFormatted("Proportional Gain = $k_p")
    TextFormatted("Integral Gain = $k_i")
    TextFormatted("Derivative Gain = $k_d")
    TextFormatted("Derivative Filter Time Constant = $τ_f")
    TextFormatted("Proportional Input Weighting = $β_p")
    TextFormatted("Derivative Input Weighting = $β_d")
    TextFormatted("Lower Output Bound = $bound_lo")
    TextFormatted("Upper Output Bound = $bound_hi")
    TextFormatted("Input = $input")
    TextFormatted("External Saturation Input = $sat_ext")
    ...
    TextFormatted("Integral Path Input = $u_i")
    TextFormatted("Integral Path Output = $y_i")
    TextFormatted("Derivative Path Input = $u_d")
    TextFormatted("Derivative Path Output = $y_d")
    TextFormatted("Free Output = $out_free")
    TextFormatted("Actual Output = $output")
    TextFormatted("Output Saturation = $sat_out")
    TextFormatted("Integrator Halted = $int_halted")
...
function GUI.draw(mdl::Model{<:LQR})
    ...
    TextFormatted("Integral Gain = $K_int")
    TextFormatted("Trim State Vector = $x_trim")
    TextFormatted("Trim Control Vector = $u_trim")
    TextFormatted("Trim Command Vector = $z_trim")
    TextFormatted("Lower Output Bound = $bound_lo")
    TextFormatted("Upper Output Bound = $bound_hi")
    TextFormatted("External Saturation Input = $sat_ext")
    TextFormatted("Reference Command Vector = $z_ref")
    TextFormatted("Current Command Vector = $z")
    TextFormatted("Current State Vector = $x")
    TextFormatted("Integrator Input = $int_in")
    TextFormatted("Integrator Halted = $int_halted")
