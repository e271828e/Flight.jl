...
export get_design_model!, generate_lookups, design_lon, design_lat
...
function get_design_model!(
    ...
    ẋp0 = ComponentVector(copy(getdata(lss.ẋ0)), Axis(xp_labels))
    ẋp0[(:EAS, :α, :β, :n_eng)] .= 0 #guaranteed by trim constraints
    ...
    #rebuild the model
    lss = LinearizedSS(ẋ0 = ẋp0, x0 = xp0, u0 = up0, y0 = yp0,
                      A = Ap, B = Bp, C = Cp, D = Dp)
