...
using LinearAlgebra, StaticArrays, ComponentArrays, Interpolations
using Logging
using Reexport
using NLopt
...
function Ldg()
    ...
    left = LandingGearUnit(
        strut = Strut(
            t_bs = FrameTransform(r = [-0.381, -1.092, 1.402], q = RQuat() ),
            l_0 = 0.5,
            damper = mlg_damper),
    ...
    nose = LandingGearUnit(
        strut = Strut(
            t_bs = FrameTransform(r = [1.27, 0, 1.4] , q = RQuat()),
            l_0 = 0.5,
            damper = nlg_damper),
