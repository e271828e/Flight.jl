...
function AirflowData(kin_data::KinData, atm_sys::AtmosphericSystem)
    #the AtmosphericData constructor accepts any GeographicLocation subtype, but it's most
    #likely that ISA SL conditions and wind will be expressed in {LatLon, Orthometric}
    atm_data = AtmosphericData(atm_sys, GeographicLocation(kin_data.pos.ϕ_λ, kin_data.pos.h_o))
