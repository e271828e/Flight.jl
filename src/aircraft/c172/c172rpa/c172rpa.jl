...
const Components = C172.Components{typeof(PowerPlant()), C172RPA.Actuation}
...
function AircraftBase.assign!(vehicle::System{<:C172RPA.Vehicle},
...
function ULinear(vehicle::System{<:C172RPA.Vehicle{NED}})
...
function YLinear(vehicle::System{<:C172RPA.Vehicle{NED}})
    ...
    @unpack components, air, dyn_data_out, kinematics = vehicle.y
    ...
    f_x, f_y, f_z = dyn_data_out.f_Gb_b
    EAS = air.EAS
    TAS = air.TAS
    α = air.α_b
    β = air.β_b
...
AircraftBase.ẋ_linear(vehicle::System{<:C172RPA.Vehicle{NED}}) = XLinear(vehicle.ẋ)
AircraftBase.x_linear(vehicle::System{<:C172RPA.Vehicle{NED}}) = XLinear(vehicle.x)
AircraftBase.u_linear(vehicle::System{<:C172RPA.Vehicle{NED}}) = ULinear(vehicle)
AircraftBase.y_linear(vehicle::System{<:C172RPA.Vehicle{NED}}) = YLinear(vehicle)

function AircraftBase.assign_u!(vehicle::System{<:C172RPA.Vehicle{NED}}, u::AbstractVector{Float64})
...
function AircraftBase.assign_x!(vehicle::System{<:C172RPA.Vehicle{NED}}, x::AbstractVector{Float64})
...
function Control.Continuous.LinearizedSS(
            vehicle::System{<:C172RPA.Vehicle{NED}},
