...
function design_lon(; design_point::C172.TrimParameters = C172.TrimParameters(),
    ...
    @assert tuple(x_labels_pit...) === propertynames(C172RPAv1.XPitch())
    @assert tuple(u_labels_pit...) === propertynames(C172RPAv1.UPitch())
    ...
    P_te, params_e2e = let
        ...
        #weight matrices
        Q = C172RPAv1.XPitch(q = 1, θ = 20, v_x = 0.1/v_norm, v_z = 1/v_norm, α_filt = 0, ele_p = 0) |> diagm
        R = C172RPAv1.UPitch(elevator_cmd = 2) |> diagm

        #feedback gain matrix
        C_fbk = lqr(P_pit, Q, R)

        z_labels = [:elevator_cmd,]
        @assert tuple(z_labels...) === propertynames(C172RPAv1.ZPitch())
...
function design_lat(; design_point::C172.TrimParameters = C172.TrimParameters(),
    ...
    @assert tuple(x_labels...) === propertynames(C172RPAv1.XLat())
    @assert tuple(u_labels...) === propertynames(C172RPAv1.ULat())
    ...
    P_φβ, params_φβ2ar = let
        ...
        R = C172RPAv1.ULat(aileron_cmd = 0.05, rudder_cmd = 0.05) |> diagm
