...
#at some point we might add f_z and f_y modes. for the f_z mode, the
#steady-state value is -g. so we need to add g to the f_z command input
...
function Systems.f_disc!(sys::System{PitchControl}, Δt::Real)
    ...
    #reset roll rate compensator upon entering direct aileron mode
    if (roll_mode != roll_mode_prev) && (roll_mode === aileron_mode)
        p_comp.u.reset = true
        f_disc!(p_comp, Δt)
    end

    if (pitch_mode != pitch_mode_prev) && (pitch_mode === elevator_mode)
        q_comp.u.reset = true
        f_disc!(q_comp, Δt)
    end

    if (yaw_mode != yaw_mode_prev) && (yaw_mode === rudder_mode)
        β_comp.u.reset .= true
        f_disc!(β_comp, Δt)
    end
...
#these are controls directly available to the pilot
@kwdef mutable struct AvionicsControlsU
...
@kwdef struct AvionicsControlsY
    eng_start::Bool = false
    eng_stop::Bool = false
...
@kwdef struct Avionics <: AbstractAvionics
    ...
    β_cmd_sf::Float64 = -deg2rad(10) #β_cmd scale factor, sign inverted
    cas::CASInner = CASInner()
end

@kwdef struct AvionicsU
    controls::AvionicsControlsU = AvionicsControlsU()
    logic::AvionicsLogicU = AvionicsLogicU()
end

@kwdef struct AvionicsY
    controls::AvionicsControlsY = AvionicsControlsY()
...
function Systems.f_disc!(avionics::System{<:Avionics}, Δt::Real,
    ...
    if flight_phase == phase_gnd
        ...
        # if ap.y.state === ap_active
        #     roll_mode = ap.CAS_roll_mode
        #     pitch_mode = ap.CAS_pitch_mode
        #     yaw_mode = ap.CAS_yaw_mode
        # else
    ...
    a_cmd, p_cmd = (1.0, p_cmd_sf) .* Float64(roll_input)
    e_cmd, q_cmd = (1.0, q_cmd_sf) .* Float64(pitch_input)
    r_cmd, β_cmd = (1.0, β_cmd_sf) .* Float64(yaw_input) #already ∈ [-1, 1]

    @pack! cas.u =  roll_mode, pitch_mode, yaw_mode,
                    a_cmd, e_cmd, r_cmd, p_cmd, q_cmd, β_cmd, p, q, β

    f_disc!(cas, Δt)

    controls_y = AvionicsControlsY(;
            eng_start, eng_stop, throttle, mixture,
            roll_input, pitch_input, yaw_input,
            aileron_cmd_offset, elevator_cmd_offset, rudder_cmd_offset,
            flaps, brake_left, brake_right)

    logic_y = AvionicsLogicY(;
            flight_phase, roll_mode, pitch_mode, yaw_mode)
