...
function generate_lookups(
    ...
    h_range::AbstractRange{Float64} = range(10, 3010, length = 2);
    ...
    filenames = string.(keys(results)) .* "_lookup.h5"
...
function optimize_pitch(ac::System{<:Cessna172FBWBase{NED}};
    ...
    if !PIDOpt.check_results(q2e_results, PIDOpt.Metrics(; Ms = 1.4, ∫e = 0.1, ef = 0.02, ∫u = Inf, up = Inf))
    ...
    if !PIDOpt.check_results(θ2q_results, PIDOpt.Metrics(; Ms = 1.4, ∫e = 0.1, ef = 0.02, ∫u = Inf, up = Inf))
    ...
    if !PIDOpt.check_results(v2θ_results, PIDOpt.Metrics(; Ms = 1.4, ∫e = 0.1, ef = 0.02, ∫u = Inf, up = Inf))
    ...
    return (q2e = q2e_results, θ2q = θ2q_results, v2θ = v2θ_results)
...
function optimize_roll(   ac::System{<:Cessna172FBWBase{NED}};
    ...
    settings = PIDOpt.Settings(; t_sim = t_sim_p2a, maxeval = 5000, upper_bounds)
    ...
    settings = PIDOpt.Settings(; t_sim = t_sim_φ2p, maxeval = 5000, lower_bounds, upper_bounds)
    weights = PIDOpt.Metrics(; Ms = 1, ∫e = 10, ef = 2, ∫u = 0.00, up = 0.00)
    params_0 = PIDParams(; k_p = 7., k_i = 0., k_d = 0.0, τ_f = 0.01)
    ...
    return (p2a = p2a_results, ϕ2p = φ2p_results)
