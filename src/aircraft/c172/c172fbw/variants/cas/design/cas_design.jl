...
function optimize_pitch(ac::System{<:Cessna172FBW{NED}};
                    point::C172.TrimParameters = C172.TrimParameters(),
                    global_search::Bool = false)

    thr_ele_MIMO = Control.Continuous.LinearizedSS(ac, point; model = :lon) |> named_ss
...
function optimize_roll(   ac::System{<:Cessna172FBW{NED}};
                        point::C172.TrimParameters = C172.TrimParameters(),
                        global_search::Bool = false)

    ail_rud_MIMO = Control.Continuous.LinearizedSS(ac, point; model = :lat) |> named_ss
