...
function generate_lookups(
    ...
    foreach(values(results), filenames) do data, fname
        C172MCAS.save_lookup(data, bounds, joinpath(folder, fname))
    end
...
function design_lat(design_point::C172.TrimParameters = C172.TrimParameters())
    ...
    #and MCAS avionics implementation for state and control vectors
    @assert tuple(x_labels...) === propertynames(C172MCAS.XLat())
    @assert tuple(u_labels...) === propertynames(C172MCAS.ULat())
    ...
    z_labels = [:φ, :β]
    @assert tuple(z_labels...) === propertynames(C172MCAS.ZLatPhiBeta())
    ...
    R = C172MCAS.ULat(aileron_cmd = 0.1, rudder_cmd = 0.05) |> diagm
