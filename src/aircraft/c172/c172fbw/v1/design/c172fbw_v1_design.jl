...
function generate_lookups(
    ...
    foreach(values(results), filenames) do data, fname
        C172FBWv1.save_lookup(data, bounds, joinpath(folder, fname))
    end
...
function design_lon(; design_point::C172.TrimParameters = C172.TrimParameters(),
    ...
    @assert tuple(x_labels_red...) === propertynames(C172FBWv1.XLon())
    @assert tuple(u_labels_red...) === propertynames(C172FBWv1.ULon())
    ...
    P_te, params_te2te = let
        ...
        @assert tuple(z_labels...) === propertynames(C172FBWv1.ZLonThrEle())
    ...
    P_vc, params_vc2te = let
        ...
        @assert tuple(z_labels...) === propertynames(C172FBWv1.ZLonEASClm())
        z_trim = lss_red.y0[z_labels]
        n_z = length(z_labels)
        ...
        R = C172FBWv1.ULon(throttle_cmd = 1, elevator_cmd = 1) |> diagm
...
function design_lat(; design_point::C172.TrimParameters = C172.TrimParameters(),
    ...
    #ensure consistency in component selection and ordering between our design model
    #and FBWv1 avionics implementation for state and control vectors
    @assert tuple(x_labels...) === propertynames(C172FBWv1.XLat())
    @assert tuple(u_labels...) === propertynames(C172FBWv1.ULat())
    ...
    P_φβ, params_φβ2ar = let
        ...
        @assert tuple(z_labels...) === propertynames(C172FBWv1.ZLatPhiBeta())
        z_trim = lss_red.y0[z_labels]
        n_z = length(z_labels)
        ...
        R = C172FBWv1.ULat(aileron_cmd = 0.05, rudder_cmd = 0.05) |> diagm
