...
function get_design_model!(
    ...
    for (old, new) in zip((:v_x, :v_y, :v_z), (:EAS, :α, :β))
    ...
    ẋp0[(:EAS, :α, :β)] .= 0 #guaranteed by trim constraints
    ...
    elseif model === :lon
        x_labels = [:q, :θ, :EAS, :α, :h, :α_filt, :ω_eng, :thr_p, :ele_p]
...
function design_lon(; design_point::C172.TrimParameters = C172.TrimParameters(),
    ...
    @assert tuple(x_labels_pit...) === propertynames(C172YControl.XPitch())
    ...
    P_red_te, params_e2e = let
        ...
        Q = C172YControl.XPitch(q = 1, θ = 20, EAS = 0.01, α = 0, α_filt = 0, ele_p = 0) |> diagm
...
function design_lat(; design_point::C172.TrimParameters = C172.TrimParameters(),
    ...
    u_labels = keys(lss_lat.u0) |> collect
    ...
    @assert tuple(x_labels_red...) === propertynames(C172YControl.XLat())
    @assert tuple(u_labels...) === propertynames(C172YControl.ULat())

    #extract reduced design model
    lss_red = submodel(lss_lat; x = x_labels_red, u = u_labels, y = y_labels_lat)
    ...
    P_ar, params_ar2ar = let
        ...
        #weight matrices
        Q = C172YControl.XLat(p = 0, r = 0.1, φ = 0.1, EAS = 0.0, β = 0, β_filt = 0, ail_p = 0, rud_p = 0) |> diagm
        R = C172YControl.ULat(aileron_cmd = 0.1, rudder_cmd = 0.01) |> diagm
    ...
    P_φβ, params_φβ2ar = let
        ...
        #weight matrices
        Q = C172YControl.XLat(p = 0, r = 0.1, φ = 2, EAS = 0, β = 5, β_filt = 0, ail_p = 0, rud_p = 0) |> diagm
        R = C172YControl.ULat(aileron_cmd = 0.1, rudder_cmd = 0.03) |> diagm
        ...
        z_labels = [:φ, :β]
        @assert tuple(z_labels...) === propertynames(C172YControl.ZLat())
        z_labels_ref = Symbol.(string.(z_labels) .* "_ref")
        z_trim = lss_red.y0[z_labels]
        n_z = length(z_labels)
