...
using Flight.FlightAircraft.C172Y
using Flight.FlightAircraft.C172Y.C172YControl
...
function get_design_model!(aircraft::Model{<:C172Y.Aircraft{NED}}, args...; kwargs...)
...
function generate_lookups(
    ...
    results = map(Iterators.product(EAS_range, h_range)) do (EAS, h)
        ...
        flaps = C172YControl.flaps_schedule(EAS)
...
#automates the design process from c172y_lon.ipynb, generating the parameters
...
function design_lon(; design_point::C172.TrimParameters = C172.TrimParameters(),
    ...
    aircraft = Cessna172Yv0(NED()) |> Model #linearization requires NED kinematics
    ...
    P_te2te, params_te2te = let lss = lss_red
        ...
        @assert tuple(x_labels...) === propertynames(C172YControl.XLonRed())

        u_trim = lss.u0
        n_u = length(u_trim)
        u_labels = collect(keys(u_trim))
        @assert tuple(u_labels...) === propertynames(C172YControl.ULon())

        z_labels = [:throttle_cmd, :elevator_cmd,]
        z_trim = lss.y0[z_labels]
        n_z = length(z_labels)
        @assert tuple(z_labels...) === propertynames(C172YControl.ULon())
        ...
        Q = C172YControl.XLonRed(; q = 1, θ = 20, EAS = 0.02, α = 0, α_filt = 0, n_eng = 0,
        ...
        R = C172YControl.ULon(throttle_cmd = 100, elevator_cmd = 5) |> diagm
    ...
    P_tv, params_tv2te = let lss = lss_red
        ...
        z_trim = lss.y0[z_labels]
        n_z = length(z_labels)
        @assert tuple(z_labels...) === propertynames(C172YControl.Ztv())
        ...
        #do not penalize θ, it will be needed to drive the change in EAS
         Q = ComponentVector(q = 20, θ = 0, EAS = 0.3, α = 0, α_filt = 0,
                        ...
                        ξ_thr = 0.1,
                        ξ_EAS = 0.01) |> diagm
        R = C172YControl.ULon(throttle_cmd = 1, elevator_cmd = 0.1) |> diagm
    ...
    P_vh, params_vh2te = let lss = lss_lon
        ...
        n_x = length(x_trim)
        x_labels = collect(keys(x_trim))
        @assert tuple(x_labels...) === propertynames(C172YControl.XLonFull())
        ...
        z_trim = lss.y0[z_labels]
        n_z = length(z_labels)
        @assert tuple(z_labels...) === propertynames(C172YControl.Zvh())
        ...
        R = C172YControl.ULon(throttle_cmd = 0.1, elevator_cmd = 0.05) |> diagm
...
#automates the design process from c172y_lat.ipynb, generating the parameters
#for all controllers at the specified design point
function design_lat(; design_point::C172.TrimParameters = C172.TrimParameters(),
    ...
    P_ar, params_ar2ar = let
        ...
        @assert tuple(x_labels...) === propertynames(C172YControl.XLatRed())

        u_trim = lss_red.u0
        n_u = length(u_trim)
        u_labels = collect(keys(u_trim))
        @assert tuple(u_labels...) === propertynames(C172YControl.ULatRed())

        z_labels = [:aileron_cmd, :rudder_cmd]
        z_trim = lss_red.y0[z_labels]
        n_z = length(z_trim)
        @assert tuple(z_labels...) === propertynames(C172YControl.Zar())

        #weight matrices
        Q = C172YControl.XLatRed(p = 0, r = 0.1, φ = 0.1, EAS = 0.0, β = 0, β_filt = 0, ail_p = 0, rud_p = 0) |> diagm
        R = C172YControl.ULatRed(aileron_cmd = 0.1, rudder_cmd = 0.01) |> diagm
    ...
    P_φβ, params_φβ2ar = let lss = lss_red
        ...
        z_trim = lss.y0[z_labels]
        n_z = length(z_labels)
        @assert tuple(z_labels...) === propertynames(C172YControl.Zφβ())

        ################################ feedback ###################################

        #weight matrices
        Q = C172YControl.XLatRed(p = 0, r = 0.1, φ = 2, EAS = 0, β = 5, β_filt = 0, ail_p = 0, rud_p = 0) |> diagm
        R = C172YControl.ULatRed(aileron_cmd = 0.1, rudder_cmd = 0.03) |> diagm
