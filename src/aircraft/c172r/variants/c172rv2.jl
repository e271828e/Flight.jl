...
Base.@kwdef struct DirectControls <: AbstractAvionics
    c1::PIDDiscrete{1} = PIDDiscrete{1}()
    c2::PIDDiscrete{2} = PIDDiscrete{2}()
end
...
const Cessna172Rv2{K, F} = AircraftTemplate{K, F, DirectControls} where {K, F <: C172RAirframe}
Cessna172Rv2(kinematics = LTF()) = AircraftTemplate(kinematics, C172RAirframe(), DirectControls())
