...
const AxesXLinear = (ComponentArrays.Axis(
    :ψ_lb, #heading (lb)
    :θ_lb, #inclination (lb)
    :φ_lb, #bank (lb)
    :ϕ, #latitude
    :λ, #longitude
    :h_e, #ellipsoidal altitude
    :p_eb, #roll rate (ω_eb_b)
    :q_eb, #pitch rate (ω_eb_b)
    :r_eb, #yaw rate (ω_eb_b)
    :v_xb, #x-body ground-referenced velocity (v_eOb_b)
    :v_yb, #y-body ground-referenced velocity (v_eOb_b)
    :v_zb, #z-body ground-referenced velocity (v_eOb_b)
    :α_filt, #filtered AoA
    :β_filt, #filtered AoS
    :ω_eng, #engine speed
    :m_fuel, #fuel load
...
function Kinematics.Initializer(x::XLinear)

    @unpack ψ_lb, θ_lb, φ_lb, ϕ, λ, h_e, p_eb, q_eb, r_eb, v_xb, v_yb, v_zb, ω_eng, m_fuel = x

    e_lb = REuler(ψ_lb, θ_lb, φ_lb)
    e_nb = e_lb #arbitrary
    q_nb = RQuat(e_nb)
...
function assign!(ac::System{<:Cessna172R}, x::XLinear)

    kin_init = Kinematics.Initializer(x)
...
function assign!(ẋ::XLinear, ac::System{<:Cessna172R})
...
function assign!(u::ULinearVector, ac::System{<:Cessna172R})
...
function assign!(ac::System{<:Cessna172R}, u::ULinearVector)
...
function assign!(y::YLinearVector, ac::System{<:Cessna172R})
