...
const Cessna172R{K, F} = AircraftTemplate{K, F, NoAvionics} where {K, F <: C172RAirframe}
Cessna172R(kinematics = LTF()) = AircraftTemplate(kinematics, C172RAirframe(), NoAvionics())
...
function Kinematics.Initializer(state::TrimState, params::TrimParameters,
    ...
    v_wOb_b = C172R.Airframe.f_ba.q(v_wOb_a) #wind-relative aircraft velocity, body frame
...
#access labels for LinearX components within the overall Cessna172R{NED} state vector
const x_labels = (
...
function assign!(u::LinearU, ac::System{<:Cessna172R})
...
function assign!(ac::System{<:Cessna172R}, u::LinearU)

    @unpack throttle, aileron, elevator, rudder = u
    ac.u.airframe.act.throttle = throttle
    ac.u.airframe.act.aileron = aileron
    ac.u.airframe.act.elevator = elevator
    ac.u.airframe.act.rudder = rudder

end

function assign!(y::LinearY, ac::System{<:Cessna172R})
...
function linearize!(;   ac::System{<:Cessna172R{NED}} = System(Cessna172R(NED())),
                        env::System{<:AbstractEnvironment} = System(SimpleEnvironment()),
