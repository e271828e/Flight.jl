...
export get_axis_value, get_button_state, get_button_change
...
Base.@kwdef struct XBoxButtons{T}
    mapping::NamedTuple{XBoxButtonLabels, NTuple{14,Int}} = default_button_mapping
    data::MVector{14,T} = zeros(MVector{14, T})
end

function Base.getindex(buttons::XBoxButtons, s::Symbol)
    data = getproperty(buttons, :data); mapping = getproperty(buttons, :mapping)
    data[mapping[s]]
end

function Base.setindex!(buttons::XBoxButtons, v, s::Symbol)
...
function get_axis_value(interface::XBoxInterface, s::Symbol)
