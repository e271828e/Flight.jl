...
wherever the two disagree. Written against the design as of D-179–D-182: detection
...
D-181's honest priors, the prior is unconditionally the quiescent sample, and the
...
gate reading `u`/`m` — is blessed by D-179, so `u`-reading localized guards are a
...
This is the discrete-driven exactness doctrine ([§2.1][s2-1], D-179) resurfacing: for edges
...
budgeted re-firing (D-181) without changing a single step.
