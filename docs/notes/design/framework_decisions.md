...
they preserve the row numbers of this log's earlier table form, which the spec
still cites as "row N" (a pending sweep converts those citations to `D-nnn`
links). Entries appear in numeric — that is, chronological — order.
...
| ID | Title | Status |
|---|---|---|
| D-001 | Hybrid causal formalism with two-tier events and projection | ratified |
| D-002 | Adopt the causal, port-based paradigm | ratified |
| D-003 | Component taxonomy: hybrid primitive, periodic discrete, assemblies | ratified |
| D-004 | Immutable value signals in a typed signal table | ratified |
| D-005 | Reject algebraic loops at build | ratified |
| D-006 | Two-stage structural feedthrough via `h_x`/`h_xu` | ratified |
| D-007 | Aggregation mechanism | superseded → D-037 |
| D-008 | Function-valued environment signals with the handle pattern | ratified |
| D-009 | Restrict deep paths to owned assembly types | ratified |
| D-010 | Structure continuous state as immutable values over a flat backing | ratified |
| D-011 | Eltype genericity on the continuous path | ratified |
| D-012 | Set-propagation tracers and SCC-based cycle diagnostics | ratified |
| D-013 | Immutable discrete state via cells, workspace, and snapshot | ratified |
| D-014 | Scoped allocation invariant, CI-enforced | ratified |
| D-015 | Fused evaluation of derivatives and outputs | ratified |
| D-016 | Uniform component interfaces via `h_x` and per-event re-decode | ratified |
| D-017 | Framework-owned simulation loop with a stepper seam | ratified |
| D-018 | Tier-2 event localization via dense output and bracketed root-finding | ratified |
| D-019 | Harmonic tick grid with virtual assemblies and rate scopes | ratified |
| D-020 | Boundary event phase iterates to quiescence | ratified |
| D-021 | Pacer: piecewise-affine wall-clock mapping with bounded debt | ratified |
| D-022 | Periphery architecture: no shared mutable model | ratified |
| D-023 | Snapshot publication via atomic acquire-release | ratified |
| D-024 | Inbound staging: per-device atomic batch cells | ratified |
| D-025 | One uniform device kind | ratified |
| D-026 | GUI write path: panels name their own ports | ratified |
| D-027 | Pacer coarse phase uses task-yielding sleep | ratified |
| D-028 | Next-snapshot wait via monotonic counter and condition variable | ratified |
| D-029 | Input trace on by default | ratified |
| D-030 | Shutdown protocol: publish, wake, unblock, join | ratified |
| D-031 | Mid-run mutation doctrine: staging and control commands only | ratified |
| D-032 | Component declaration: trait layer with probe-checked schema authority | ratified |
| D-033 | Declaration inventory: by-value, by-type, by-allocation registers | ratified |
| D-034 | Contract visibility: declared fields are public | ratified |
| D-035 | Stores and views: components read zero-copy view bundles | ratified |
| D-036 | Table mechanics: stage returns are NamedTuples of port values | ratified |
| D-037 | Aggregation by explicit summing junctions | ratified |
| D-038 | Snapshot and log: derived trajectory, primary trace header | ratified |
| D-039 | Assembly declaration is type-based | ratified |
| D-040 | Slash-string paths as the canonical path form | ratified |
| D-041 | Dedicated `exports` for assembly faces | ratified |
| D-042 | `rates` declaration on immediate children only | ratified |
| D-043 | Computed exports via ordinary code and `faces` | ratified |
| D-044 | Slot exclusivity and the write-surface rule | ratified |
| D-045 | Periphery input semantics: derived liveness, conditioning, mappings, edge logic | ratified |
| D-046 | Face names are opaque string tokens; slash is reserved for structure | ratified |
| D-047 | Stage widgets on interaction events, not every pass | ratified |
| D-048 | Three-stratum build pipeline: structure, schedule, activation | ratified |
| D-049 | Standalone `build(world)` artifact wrapped by `Simulation` | ratified |
| D-050 | Probe every user function once at build, at nominal `T` | ratified |
| D-051 | Synthesize probe inputs via a `probe_value(::Type)` fallback chain | ratified |
| D-052 | Scope Dual probing to each activation's executable set | ratified |
| D-053 | Bake one always-on conformance check at every table write | ratified |
| D-054 | Producers determine activation types; consumers stay generic | ratified |
| D-055 | Require strict `local_types` declaration for cross-stage cells | ratified |
| D-056 | Uphold the two-kind taxonomy against the integrate-and-dump challenge | ratified |
| D-057 | Batch declarative-check violations; abort on first user-code exception | ratified |
| D-058 | Diagnostics as structured values under one `BuildError` carrier | ratified |
| D-059 | Catch runtime failures at one boundary site into `StepError` | ratified |
| D-060 | Graceful termination is model state, never an exception | ratified |
| D-061 | `resolve` walks declared types to enforce the generic-boundary rule | ratified |
| D-062 | Tooling commitments: face predicates, build printer, component library | ratified |
| D-063 | Conditions are path-addressed sparse overlays on `init_*` defaults | ratified |
| D-064 | Compose per-component init by pull, via fragment functions | ratified |
| D-065 | Fragments form a lazy inert tree, resolved against the `Build` | ratified |
| D-066 | Two application registers: specialized `apply!` vs dynamic entry-list walk | ratified |
| D-067 | Boundary zero runs the macro-sequence with an empty integrate | ratified |
| D-068 | Enforce slot totality at the init/commit service boundary | ratified |
| D-069 | Trim problem spelling: NamedTuples, residual vector, exact AD Jacobians | ratified |
| D-070 | Trim service: in-house dense LM behind a swappable backend | ratified |
| D-071 | Mount trim as an implicitly specified condition via `at` | ratified |
| D-072 | Linearization surface: three selector lists, one chunked Dual pass | ratified |
| D-073 | Companion sketches carry the settled condition-algebra design | ratified |
| D-074 | Hand off component-function arguments as one named bundle | ratified |
| D-075 | Name flow/update/output stages by letter and dependence class | ratified |
| D-076 | Name declarations `input_types`/`output_types`/`local_types` by register | ratified |
| D-077 | Allocate workspace via a per-activation `workspace` method | ratified |
| D-078 | Treat input entries as face constraints checked by subtyping | ratified |
| D-079 | Type declarations concretely, resolved by an activation leaf walk | ratified |
| D-080 | Keep Tier-2 event detection pace-independent | ratified |
| D-081 | Treat `t*` as a boundary, not a frame | ratified |
| D-082 | Define guard conditions against a per-event baseline | ratified |
| D-083 | Bind output-device reads to snapshot paths, not just faces | ratified |
| D-084 | Drop the unconnected-output warning | ratified |
| D-085 | Unpack `Tuple`/`NamedTuple` fields as container children | ratified |
| D-086 | Compile the executor's schedule into unrolled statically-typed entries | ratified |
| D-087 | Freeze the device roster as a build-validated immutable value | ratified |
| D-088 | Define the run lifecycle: built → initialized → running → stopped/errored | ratified |
| D-089 | Route supervisor gains and resets through ordinary ports | ratified |
| D-090 | Return handler updates as bundle-law NamedTuples | ratified |
| D-091 | Override `t_end`/`stop_on` per run at `run!` | ratified |
| D-092 | Normative diagnostic kind table (Appendix C) | ratified |
| D-093 | Spawn device tasks per run, not per attach | ratified |
| D-094 | Close the state-leaf vocabulary to plain scalars and `SArray`s | ratified |
| D-095 | Prefix the read-selector family with `get_` | ratified |
| D-096 | Define the harness register: `stage!`/`latest`/`step!` duration | ratified |
| D-097 | Split device shutdown failures into two diagnostic kinds | ratified |
| D-098 | Resolve selectors against a source before client policy | ratified |
| D-099 | Spell the CI activation invariant with a canonical probe scalar type | ratified |
| D-100 | Freeze `u` at round start for within-round event visibility | ratified |
| D-101 | Implement replay as the ordinary loop with two substitutions | ratified |
| D-102 | Author-owned device loop inside a framework-owned bracket | ratified |
| D-103 | Detect a binding's sides by method presence at attach | ratified |
| D-104 | Coalesce staged writes by CAS merge with per-attachment positional shape | ratified |
| D-105 | Split device-side bad-datum handling into tolerated garbage and propagated crashes | ratified |
| D-106 | Freeze the device roster for the duration of a run | ratified |
| D-107 | Match trace record density to batch density, not surface width | ratified |
| D-108 | Gate stopped-sim services by input-derived lifecycle preconditions | ratified |
| D-109 | Fix device identity, roster admission and calling-task topology at attach | ratified |
| D-110 | Give `trim!` an explicit `t0` argument and state its recording clear | ratified |
| D-111 | Fold `project` into the always-on conformance check | ratified |
| D-112 | Add `events` to the tier-consistency markers | ratified |
| D-113 | Close four kindless diagnostic rules with new payload fields | ratified |
| D-114 | Order snapshot publication before the boundary counter increment | ratified |
| D-115 | Source the bundle law's remaining probe fields `t` and `ws` | ratified |
| D-116 | Expose `phase_bodies(sim)` as the zero-allocation invariant's measurement seam | ratified |
| D-117 | Extend declarations and stages via explicit per-name import | ratified |
| D-118 | TrimProblem authoring surface | ratified |
| D-119 | Add `Simulation(build::Build; …)` as a second constructor | ratified |
| D-120 | Reconcile rigs with abstract-at-root via stub children | ratified |
| D-121 | Partition the cell/store vocabulary and bless "staging cell" | ratified |
| D-122 | Resolve de-polysemy by giving each overloaded term one owner | ratified |
| D-123 | Add a non-normative Appendix D glossary | ratified |
| D-124 | Widen §14.7's `reads` grammar to the full load-bearing selector set | ratified |
| D-125 | Admit `get_face` to the load-bearing `reads` set | ratified |
| D-126 | Give trim-commit events a report channel | ratified |
| D-127 | Add a deployment block to the trace header for replay validation | ratified |
| D-128 | Define `to_boundary` as the frame-entry boundary index | ratified |
| D-129 | Restate the two-notation rule as directional (structure vs contract) | ratified |
| D-130 | Scope `resolve`'s generic-boundary duty by register (structural/load-bearing/diagnostic) | ratified |
| D-131 | Apply the round-4 consistency sweep (findings 4–11) | ratified |
| D-132 | Treat operator interrupt (Ctrl-C) as a control-plane stop, not a failure | ratified |
| D-133 | Split spec-invoked numeric constants into deployment parameters vs owning-section defaults | ratified |
| D-134 | Declare the interactive write-surface class via a binding marker method | ratified |
| D-135 | Scope the activation cache to immutable compiled artifacts, keyed on the Build | ratified |
| D-136 | Unify diagnostics and liveness heartbeat into one per-writer diagnostic cell | ratified |
| D-137 | Bound snapshot-log retention by count, with amortized doubling stride | ratified |
| D-138 | Make device `init!` an explicit, bracketed step of the run-start protocol | ratified |
| D-139 | Give environment field handles a value-level constructor to prevent drift | ratified |
| D-140 | Add a three-rung remedy ladder for artificial algebraic-loop dependencies | ratified |
| D-141 | Continuous state resets are events, owned by the reimplemented PIVector | ratified |
| D-142 | Stage code must be total over type-valid inputs | ratified |
| D-143 | Add a `Constant` source block for zero-contributor aggregates | ratified |
| D-144 | Rename the computed-exports helper `faces` to `passthrough` | ratified |
| D-145 | Deduplicate pass-through `except` lists with a shared feed-list idiom | ratified |
| D-146 | Rename `faces`/`selectors` to `claims`/`reads` on the binding interface | ratified |
| D-147 | Split the sweep into static interior and boundary variants | ratified |
| D-148 | Select converters per leaf by resolved condition-shape type | ratified |
...
| D-156 | Legalize the three degenerate trim/integration shapes | ratified |
| D-157 | Check for nonfinite `x` immediately after integrate | ratified |
| D-158 | Pin the backend seam to one required `solve` signature | ratified |
| D-159 | Define `warning (service)` as a sixth severity | ratified |
| D-160 | Rename `report` to `report!` | ratified |
| D-161 | Grow the naming audit to the full register-violation set | ratified |
| D-162 | Adopt per-eltype homogeneous cell stores over per-instance | ratified |
| D-163 | Ban `==` for separately-compiled float comparisons | ratified |
| D-164 | Reject components that declare nothing and define no stage | ratified |
| D-165 | Split output-stage returns into public `y` and private `w` | ratified |
| D-166 | Mandate `Type{T}` output signatures on continuous producers | ratified |
| D-167 | Mandate `Type{T}` input signatures under the permissive reading | ratified |
| D-168 | Root-slot fan-out tolerance combines by meet, not agreement | ratified |
| D-169 | `y_x`/`y_z` carry stage-1 ports only, auto-published excluded | ratified |
| D-170 | Split assembly connections into child/input/output declarations | ratified |
| D-171 | Rename `passthrough` to `input_passthrough` | ratified |
| D-172 | Rule "face" kind-blind, defined at first use | ratified |
| D-173 | Fuse the discrete state letter `z` into `x` | ratified |
| D-174 | Re-class the GUI as an ordinary enumerated writer | ratified |
| D-175 | Re-scope `gui = true` to a run-scoped attachment | ratified |
| D-176 | Unify trace retention on one sparse record format | ratified |
| D-177 | Re-found the periphery on mandatory roots plus declared traits | ratified |
| D-178 | Reaffirm component-side rejections against the periphery's new idiom | ratified |
| D-179 | Derive detection policy from the guard's return type | ratified |
| D-180 | Bank per-event `direction` as an unbuilt guarded addition | ratified |
| D-181 | Replace once-per-boundary firing with budgeted re-firing | ratified |
| D-182 | Add a θ=0 validation probe to the localization trigger | ratified |
| D-183 | Retire workspace poisoning | ratified |
| D-184 | Fold `Group` into the spec as an ordinary library component | ratified |
| D-185 | Adopt the phased, two-register sample-time declaration | ratified |
| D-186 | Legalize absolute declarations in any scope via anchors | ratified |
| D-187 | Make the bound schedule a named artifact with exact grid diagnostics | ratified |
| D-188 | Construct `TrimProblem` by keyword everywhere | ratified |
...
DAE/SDE/per-step hook. Tier-2 detection is pace-independent (D-080); guard
conditions are normative — positive = holding, edge semantics (D-082).
...
all-inputs `h_xu`, named per D-075); component split as the refinement.
...
included), and additionally receive state views (D-035); single computation
...
`h_x`, with selective auto-publication of declared state/mode fields (D-035);
...
**Spec.** §4.2, §5.3, §7.4, §8.6, §11.3, Appendix D
...
- *Superseded position — identity publication of state and modes as the
  ...
  design of record):* retired because contract visibility (D-034) makes
...
- *The `unlisted` motivating case — RNG state feeding the component's own
  update:* dissolved entirely once D-035 gave every function direct `x` views,
...
(D-080 intact), and never a `StepError`; probes evaluate the raw interpolated
state (off-manifold like RK stages), projection running at the `t*` boundary
where §8.6's edge checks read it — a projection that undoes the crossing costs
...
(D-074), single source of truth, no stored `Δt`-derived parameters.
...
- *Superseded position — a `comp.Δt` virtual property (the earlier design of
  ...
  component fact (D-074).
- *Atomic assemblies, incl. opt-in:* coarsened schedulable unit → §5.4
...
per-event baselines in loop state (D-082); due `g` updates run after
...
- *Single pass per boundary:* cascade latency N·h — step-size-dependent
  semantics, the §2.2 `f_step!` footgun class, made common by §3.1 externalized
  FSMs.
...
slot exclusivity (D-044); levels-never-deltas doctrine; mappings pure, on the
...
**Position.** Next-snapshot wait: monotonic boundary counter (D-081) +
...
init/trim = stopped-sim services (§14); mid-run intervention command = guarded
...
- *Inference-by-evaluation as schema authority:* error locality inverts —
  ...
  homeless. The five §11.4 walkthroughs traced under inference-by-evaluation,
...
wiring check `producer_face <: entry` (D-078);
`output_types(::C)`/`local_types(::C)` plain concrete nominal declarations on
both tiers, per-activation cell types from the framework leaf walk (D-079);
`workspace` registered by allocation (D-077); `events(::C)` ordered + per-event
...
**Spec.** §4.2, §11.1, §11.2, §12.7

**Rationale.** The inventory is self-classifying by register: by value
`init_*`, by type `*_types`, by allocation `workspace` (D-076). Contract
...
exempt (by-allocation, D-077) — because §12.7's entry typing derives the bundle
...
- *Superseded position — `outputs(::C, ::Type{T})` on continuous components
  ...
  = deliberate non-participation; retired with the `T`-signature (D-079).
...
- *Names-only input contracts:* lose wiring-time type errors and standalone
  checkability.
- *Per-stage output lists:* stage membership is internal, §4.2.
...
no outputs; intermediates declared via strict `local_types` (D-055) — table
...
(D-074); the table holds produced signals only, never transported ones (one
...
- *Superseded position — state-free evaluation prototypes with identity
  transport (the earlier design of record, D-015/D-016 as argued):* the
...
**Rejected.**
- *Reduce-ports (D-007, reversed):* the declaration vocabulary's last wrapper;
...
(recomputable by replay), so D-029's anti-decimation argument does not reach
...
**Rejected.**
- *Auto-bubbling:* forgotten wire silently promoted to a live root slot — §11.4
...
write-surface rule (§9.3): an entry reaches a slot iff the face is inside the
writer's surface, else discard + warn — under the D-106 roster freeze every
...
**Spec.** §9.3, §9.6, §9.7, §15.4

**Rationale.** Recorded only through the rejections below.

**Rejected.**
- *Cross-device attachment-order precedence as conflict policy:* resolves races
  the §15.4 cast shows nobody wants — every dual writer is a stream shadowed by
  a mirror.
...
- *Unclaimed-register opportunism "for any device":* reinstates the rejected
  attachment-order policy; breaks §9.7's stays-put and sole-writer arguments.
...
- *Joystick-as-component and root-level `PilotInterface`* (§15.4): replay
  same-build, single audit point, no natural home in `World`. Annotation (from
  §15.4's inline treatment, migrated): devices-as-components also duplicates
  §10.4's device lifecycle in component vocabulary and costs the drain its
  ...
  — and the rejection rests on these invariants, not on §10.5's clock
...
- **Bundled command faces** (`pilot_inputs` as one struct port): kills
  ...
  periphery's atomic unit on the write side too, §4.3 — and the routing
...
tokens; the periphery speaks face names on the *write* side (D-083); `exports`
...
cannot occur under the D-106 freeze, liveness is baked once per run, and the
...
- *Inputs declared by value à la `init_x`:* reads as an unwired-input default —
  §6.1 contradiction; every leaf pays for a root-slot-only need; fan-in faces
  need an agreement rule; reopens D-033.
...
rescinded by D-135: the cache holds immutable artifacts only and lives on the
...
two §2.1 forms: `Bool` for predicates, the nominal scalar for continuous
...
activations accept `{T, Float64}` with zero-partial embedding (D-079).
...
marking) is also rejected — zero information (D-078).
...
- *`input_types(::C, ::Type{T})`:* forces the consumer to declare its
  ...
  same face; D-033's exact-`Float64`-face check was already the only coherent
  consumer statement.
...
(supersedes D-034's observation exception; adds §11.4 walkthrough 5); concrete
nominal on both tiers, same activation leaf walk as `output_types` (D-079).
...
- *Superseded position — the earlier `T`-signature spelling `locals(::C,
  ...
  under `Dual`:* retired with the `T`-signature (D-079).
...
(§15.5): the kinds are time bases (sweep-driven vs. tick-driven), and
...
**Rejected.**
- *Termination-by-exception:* aborts a boundary §10.4 is built on completing;
  models never state their terminal dynamics.
- *`stop_when` predicate closures:* opaque, unserializable, a public snapshot
  API — `user_callback!` redux. Annotation (from §13.5's inline treatment,
...
- *Scanned terminal types / `terminal` event flags:* action at a distance: deep
  ...
  about it.
- *Control-plane capability components* (§10.1): components live inside
  boundary semantics.
- *Observation-by-path:* load-bearing observation must speak the contract;
  diagnostic observation sees everything — §6.1's knowledge rule applied to
  reads.
...
instances, enforcing §6.1's generic-boundary rule at the primitive
(past-generic segment = diagnostic even where the instance resolves;
register-scoped by D-130 — structural and load-bearing registers enforce, the
...
**Rejected.**
- *Instance-only walk:* blind to generic holding — §6.1 unenforceable where
...
- *Per-site hand-written junctions:* prices §6.2's explicit-junction doctrine
  dishonestly.
...
`x`/`m`/`z` fields (§11.6 path + field) and root slots (by face) on the
...
- *`initialize(::C, spec)` schema + assembly routing:* call-tree composition
  reborn: spec tree mirrors assembly tree = D-039 two-artifact drift; spec
...
included); locality law = §6.1's, third instance (own fields, declared
children, own faces; deep `at` only within owned concrete subtrees) — absolute
paths are compiled derivatives, so §13.5's observation-by-path rejection is
untouched.
...
`apply!` (`Getter{P}` lenses, unrolled baked stores, zero-alloc; §12.5-style
...
per-shape codegen, allocation fine per §7.5) for one-shot init; compiled
...
- *Per-write convert decisions:* the converter is a resolution-time fact;
  §12.5's no-convert-on-write stands for table cells.

### D-067 — Boundary zero runs the macro-sequence with an empty integrate

**Status.** ratified

**Position.** Boundary zero = the §8.6 macro-sequence with an empty integrate:
...
interval-alignment taught contract (sibling of §15.5's boundary-sampling line):
...
**Spec.** §8.6, §14.5, §15.2, §15.5

**Rationale.** Recorded only through the rejections below.

**Rejected.**
- *Condition-authoritative boundary zero:* no events/updates: delays the
  identical firings one step while hiding non-quiescence — §9.7's
...
- *Update before the sweep or republish-from-`z⁺`:* stale-table sampling or
  Mealy update-feedthrough: same-boundary circularity, kills §8.6's structural
  termination.
...
- *Face-declaration defaults:* condition data inside the wiring contract;
  reopens §9.3 bare-types and the competing-defaults problem.
- *Silent zero-fill of uncovered slots:* the §12.3 probe-value leak — a
...
**Rejected.**
- *External NLS packages:* heavy dep for ~100 lines; §8.2 stepper precedent;
...
(§7.1 discharged); evaluation = one chunked Dual pass on per-invocation scratch
...
§14 algebra and §14.9 mounting (printed trees and flattened entry lists);
declaration-by-initial-value upheld (§11.2); sampled-data Dual activation
recorded unbuilt (§14.10); §6.2's `SumJunction` dispatches on its
unparametrized type constructor.

**Spec.** §6.2, §11.2, §14, §14.9, §14.10

**Rationale.** Recorded only through the rejections below.

**Rejected.**
- *Split-form sketch files and separate `navsensors.jl`/`imu.md` notes:*
  retired, content absorbed into §15.5.
- *`init_*` as types + `probe_value` synthesis:* defaults are the §14.1 overlay
  base; the §14.6 probe-value barrier; a per-field two-register protocol,
  §14.2. The `init_*`-as-types grounds spelled out (§11.2): the condition
  substrate needs an authored value under every leaf (§14.1's overlays fall
  back leaf by leaf and the compiled store writers bake `merge(defaults,
  overlay)`), a synthesized initial state crosses §14.6's probe-value barrier
  ...
  §14.2 kills for `initialize` specs, aggravated in Julia by types being
...
- *Keyword arguments via `Base.kwarg_decl` reflection:* load-bearing seam on an
  internal binding — the §8.1 `task_local_storage` lesson.
...
- *A `comp.Δt` virtual property:* D-019 — the period is a schedule-position
...
names to all tier markers, `events` included, D-112); three-level narrowing
...
- *A tier-neutral stage pair:* no honest neutral state letter; tier-distinct
  names double as cheap drift checks. Superseded on this point by D-173 —
...
dissolves; tier comes from declaration shape (D-079), not from the method
signature.
...
- *Concrete-nominal allocation + framework `similar`-based re-scalaring
  (D-079-style):* no schema stake here; reconstruction cannot cover non-array
...
- *Superseded position — `init_workspace` by value (earlier design of record):*
  a workspace is not memory that conditions overlay, and no §11.2 by-value
...
**Rationale.** Abstract entries = structural substitutability (§4.4 field
...
**Rejected.**
- *Exact-equality-only status quo:* welds §4.4 consumers to one concrete
...
- *Symmetric `T` as genericity envelope:* a per-leaf marking that is a constant
  ...
  false-affordance pins; the predictive reading remains impossible per D-054.
- *Phantom root producer with `output_types`-style slot declarations:*
  re-enumerates what exports already are — D-043's `add_input!` reborn;
...
(frozen-exact by typing rule; tier from declaration shape, §11.5); root slots
...
schema-visible freezing a recorded §14.10 door.

**Rejected.**
- *Superseded position — the `T`-signature on the output side (D-033/D-055,
...
identically in every execution mode, its sweep cost absorbed as §8.7 pacer debt
...
- *Superseded position — degrade to Tier-1 under real-time pacing (earlier
  ...
  unpaced trajectories, violating §8.7's bit-identical invariant; the "blowing
...
**Rationale.** At `t*` the full §8.6 iteration runs with once-per-event scoped
per boundary, snapshot published, §10.3 boundary counter incremented, `stop_on`
...
**Rejected.**
- *Frame-only publication:* contradicts §13.5's snapshot-at-the-crossing
  promise.
- *Drain at `t*`:* input timing dependent on localization arithmetic — replay
  indeterminism.
- *Per-`t*` pacing:* wall placement below pacer resolution; the §8.7 invariant
  concerns trajectories.
- *A frame counter in §10.3:* devices assuming fixed-`h` wake spacing — nothing
  settled does.
...
conditions fire at `t₀`, §14.5 derived); opposite direction = second event with
...
- *Suppressing `t₀` firings:* §14.5's anti-diagnostic insurance.
...
as the checkable fraction of semantics; §13.5's observation-by-path rejection
...
models carry many observation-oriented ports no wire consumes (§9.2 blesses
exactly that), and D-083's path-bound readers attach post-build, so "unused" is
undecidable at build.

**Spec.** §6.1, §9.2, §9.7, §13.2, Appendix C

**Rationale.** Such a warning fires on every honest port and poisons the sole
warning stream (§9.7's anti-diagnostic lesson); the hazard it would guard (a
...
instantiation via §11.8 generic holding, wires generated by comprehension
(arity-via-computed-contracts at structure scale; §14.9 swarms and
...
- *Homogeneous-`NTuple`-only:* the per-element complexity objection was hollow
  — element types are static either way; named and mixed rosters are the §14.9
  case, user call.
...
**Position.** The compiled executor (§12.7): schedule data compiled per
activation into a concretely-typed entry tuple over statically typed storage,
walked by compile-time unrolling — §7.5/§12.5/§5.1 are reachable only under
full specialization.

**Spec.** §5.1, §7.5, §12.5, §12.7
...
- *Vector-of-abstract entries:* per-entry dynamic dispatch boxes stage returns
  — kills §7.5 and its canary role, unfolds §12.5's check; union splitting
...
- *Type-erased call tables (`FunctionWrapper`-style):* same specialization
  ...
  load-bearing seam on internal ABI — the §8.1 lesson.
...
**Position.** Device roster representation and provenance (§9.3), as narrowed
by the D-106 freeze: the roster (entries, claims, order) is a plain immutable
...
- *Superseded position — attach/detach legal at any time, an atomically
  ...
  superseded wholesale by the D-106 roster freeze.
...
- *Mid-run claim release on task death:* the freeze adopts the reverse: see
  D-106's reversal note.

### D-088 — Define the run lifecycle: built → initialized → running → stopped/errored

**Status.** ratified

**Position.** Run lifecycle (§10.6): built → initialized (`init!` ran boundary
...
orthogonal to the run lifecycle, tasks per-run per D-093); between `step!`
...
during `step!` through the ordinary §10.4 tail, and `step!` returns the frames
*actually* advanced; `t_end`/`stop_on` re-bindable per run at `run!` (D-091);
`errored` terminal (D-059).

**Rejected.**
- *`run!`-only surface:* kills the test-harness advance-assert register and the
  REPL fly-inspect-continue register — neither is a §10.5 script.
- *Implicit auto-`init!`:* which condition? silent defaults are §14.6's
  rejected zero-fill.
- *Resumable errored sims:* D-059 — stores may be mid-boundary.

### D-089 — Route supervisor gains and resets through ordinary ports

**Status.** ratified

**Position.** Supervisor idioms (§15.2): scheduled gains are input ports —
...
the *output stage* with `g` storing the matching `z⁺` (Appendix A entry; reset
...
- *Writable parameter store / `Ref`-mutable component parameters:* invisible to
  log, trace, replay and the feedthrough graph — D-031's raw-poke rejection
...
- *Reset-in-`g`-only as the idiom:* publishes the stale command at the
  engagement tick — the bump bumpless transfer removes; §8.6's `z⁻¹` discipline
  makes the output stage the only same-tick path, D-067.

### D-090 — Return handler updates as bundle-law NamedTuples

**Status.** ratified

**Position.** Handler returns are NamedTuples under the bundle law (§5.2,
§12.5): `(; x, m)`, a key present iff the store exists AND the handler updates
...
**Rejected.**
- *Positional pair `(x⁺, m⁺)`:* reintroduces the padding D-074 bans on
...
at `run!` (§13.5, §10.6): the `run!` argument binds that run only, nothing on
...
**Rejected.**
- *Ctor-only binding:* a different stop policy across §10.6's re-run/`step!`
  cycles costs a rebuild.
- *Root-type-declared policy:* D-060 stands — the override moves binding
  *later* along the deployment axis, not into the model; the root-declared
  *default* variant remains the §16 residual.
...
**Rationale.** D-058's missing deliverable and the acceptance-test contract —
adding a kind is a decision-log entry.

**Rejected.**
- *Illustrative prose enumeration ("roughly one per rule"):* D-058 makes tests
...
**Position.** Device tasks are per-run artifacts (§9.1, §10.4, §10.6): `run!`
...
**Spec.** §9.1, §9.6, §10.2, §10.3, §10.4, §10.6, §13.4

**Rationale.** §10.4 joins all tasks at every stop with `shutdown!` releasing
...
gate, and first-boundary synchronization is §10.3's counter-plus-condition
predicate wait; `should_close` and the §10.2 liveness heartbeat are run-scoped,
...
true` mere ensure-rostered sugar (D-109) — otherwise the loop runs on the
calling task (preserving §13.4's synchronous rethrow and inline threading of
...
next `run!`); the write/read paths while stepping are D-096's harness register;
`init!` discards staged batches along with trace and log.

**Rejected.**
- *`Event` start gate "as today":* §10.3's rejected primitive — the `io_start`
  reset race, re-imported for the re-run cycle §10.6 newly supports.
- *Persistent tasks parked across stops:* `while running(handle)` false from
  the first iteration; contradicts §10.4's guaranteed `finally shutdown!`;
...
**Position.** State leaf vocabulary closed (§7.1): `init_x` leaves are plain
...
off-manifold state is visible to §8.4's probes by construction; domain
...
§14.3, `capture`'s gather, handlers' own constructors).
...
- *Invariant-carrying leaves with constructors run on read:*
  ...
  `project` view-redundant yet buffer-necessary, §8.4's off-manifold probes
  impossible.
...
(§14.4): `get_state`/`get_deriv`/`get_output`/`get_local`/`get_slot`/`get_face`
...
**Rationale.** `get_face` gives §9.2's integration register its spelling
...
(completed by D-125: `get_face` admitted) within owned scopes (no `get_local` —
export instead), diagnostic readers the whole family (within D-098's later
...
- *`stop_on` as a family client:* §13.5 forbids path-addressed termination —
...
**Position.** The harness register (§10.6, §9.3, §9.2): `stage!(sim, "face" =>
value, ...)` = task-free staging from the calling task into the interactive
register (D-044's derived surface) — traced, surface-checked, drained *last* at
...
exclusive with `frames`); together the write/read halves of D-088's
...
- *Direct slot pokes from the calling task:* §2.2/§10.5's rejected
  unrecorded-mutation register — breaks replay.
- *Leaving reads to device handles or the log:* the log can be off, a handle
  needs a device, and D-088's "inspect" must be spellable.
- *A public closure-evaluation API:* §13.5's rejected cost — an inspection
...
**Spec.** §10.4, §13.2, §13.4, Appendix C

**Rationale.** §10.4's committed abandoned-join warning and device-crash log
join §13.2's closed inventory per D-092's rule; two kinds, not one, because the
payloads differ in the load-bearing field (`cause`) and §13.4's domain
...
- *A single merged device-failure kind:* union payload; blurs abandoned-join vs
  caught-crash, which D-058's kind+payload test binding must distinguish.
- *Leaving them prose-only warnings:* a committed warning with no kind is
  D-092's rejected "unwritable-against" state.
...
D-083's client split rides on top unchanged, D-095's family stays closed and
identically spelled.

**Rejected.**
- *Client-only validation:* D-095's original "diagnostic readers admit the
...
- *Runtime resolution failure:* defers the error §9.2/§15.4 deliberately moved
  to attach.
- *Snapshots carrying stores to make such bindings resolvable:* D-038 stands —
...
**Rationale.** Activations are keyed by concrete scalar types (D-052) and a
bare `Dual` `UnionAll` cannot key one, be walked to, or answer `zero(T)`; the
width is arbitrary (CI pins genericity, not a Jacobian — §14.10 chunks at its
own widths).

**Rejected.**
- *Bare `Dual` in the invariant:* §12.4 contradicted its own caching paragraph
  twelve lines down.
...
**Spec.** §5.3, §8.6, §9.2, §13.4

**Rationale.** Immutable cell values make early reference-loading a zero-copy,
zero-allocation capture (§9.2's publication trick applied within a boundary);
cross-component handler order is thereby semantically unobservable and is fixed
(executor component order, declaration order within a component) only for §13.4
...
and BEFORE the boundary-zero sequence — §14.5's end-of-sequence capture
...
header's schemas (D-107 — once, off the loop, so the replay drain applies
...
- *A `run!(sim; replay = trc)` flag:* replay replaces `init!` and swaps the
  drain source — a lifecycle entry, not a run option; muddies §10.6's
  mandatory-`init!` rule.
...
- *Header = the authored sparse condition:* §14.5's old wording — silently
  replays a different initial state after an `init_*` default edit, D-038's
  trap.
...
inline on the calling task (§9.1); heartbeat timestamps ride inside handle
primitives, so liveness needs no loop ownership; the two author obligations
(`running(handle)` predicate, interruptible blocking) are taught (Appendix A)
...
(output half: §14.4 selectors compiled to one gather at attach); sides detected
by method presence at attach (the §11.5 declaration-shape idiom; `hasmethod` is
public stable API at a service point, not D-074's internal-reflection seam),
neither side = attach error.

**Spec.** §9.4, §9.6, §11.5, §14.4
...
its generic `map_input` owning §9.4's conditioning helper; `map_input` purity
taught: cross-datum state lives in the device struct, arrives inside the datum.

**Rejected.**
- *Abstract binding-type taxonomy:* resurrects the §9.6 kill one level down; a
...
attach-compiled drain scatter (mirror of §9.2's gather), and `stage!`-side
...
**Rationale.** The interactive register gets the same treatment under the D-106
...
the drain checks nothing (D-106); trace records match batch density (D-107);
...
- *Name-keyed interactive batches, the earlier design of record:* justified
  only by the mid-session-mutable surface the D-106 freeze removed.
...
- *Author-built total tuples:* a padding form — the D-074 / handler-return-law
  disease.
...
implementation freedom, not an obligation — the §12.7 per-configuration compile
trade); device death mid-run is not detach — the task dies, the heartbeat
reports it, claims and roster entry persist to run end (**reversal of D-087's
...
(stop → `detach!` → `init!`, or `replay!`-to-end + `run!` continuation, §10.7);
§10.5's doctrine reaches final form — the running periphery stages writes and
...
moves no surface; a dynamic reader list touches only §10.3 wakeups, heartbeat
...
**Rejected.**
- *The earlier attach-anytime design of record:* D-087's republished-roster
  machinery, drain-time derived-surface membership checking via
  `StaleInteractiveEntry`, §9.7's mid-drag claim-transition policy, per-attach
  thread-budget re-checks — all priced for a hypothetical workflow.
- *Mid-run claim release on device death:* the reversed D-087 rejection — under
...
interactive-active frames, inside the §7.5 retention carve-out the log already
occupies — so trace size tracks information, not surface width, preserving
D-029's two-orders-below-the-log budget at every world scale.
...
claims); replay pays the inverse conversion once, up front, in the §10.7
...
requires a non-running simulation, pause included (the D-106 doctrine — the
...
**Rationale.** `errored` is terminal for all four (D-059) — a captured
...
`attach!`/`detach!`-while-running — closing the kind D-106's gating text
lacked.
...
**Spec.** §9.1, §9.3, §9.6, §10.4, §10.6, Appendix B
...
runs that device's loop body inline in the identical §9.6 wrapper, else the
loop runs on the calling task; the affine device is outside §10.4's join and
...
— D-067's register applied to the second init-service entry point, never a
`TrimProblem` field (problems stay time-free, §14.9's lift untouched); default
`0.0` matching `init!` (one rule for both entry points), resume-at-time spelled
explicitly via `capture`'s returned `t` passed as `t0`.

**Spec.** §10.6, §14.5, §14.6, §14.8, Appendix B

**Rationale.** The commit's side effects stated where the reader looks (§14.8):
an `init!` in every respect — boundary zero, §14.6 pre-write totality,
guards-at-commit, `t0` anchoring, and the §10.6 three-item clear (trace, log,
staged batches).
...
- *Leaving `events` outside the rule:* a discrete leaf declaring `events` is
  illegal by §5.2/§3.2/Appendix B's testimony yet checkable nowhere, and
...
value in hand, violated constraint; service batch per D-057's declarative
...
cluster — discharged, D-118); `ConditionNodeMisuse` for §14.2's mixed-`merge`
...
**Spec.** §6.1, §11.4, §11.8, §12.1, §14.2, §14.8, Appendix C
...
boundary index is carried *in* the snapshot (with `t`) — what §8.4/§13.4's
...
- *Counter as loop state only:* unreachable from a retained snapshot — breaks
  §8.4/§13.4's counter-plus-recorded-`t` boundary indexing.
...
deployment, D-051's strict probe-scoping applied to the clock, matching the
...
because D-077's allocator reads only the instance and the scalar, deriving
nothing from layouts — no Stratum C dependence created, nothing moves between
strata (D-048).

**Spec.** §12.1, §12.3, §12.4

**Rationale.** §12.4's activation-time allocator invocation clarified as a
...
- *Workspace allocation left as an activation-only (Stratum C) act:* the
  ...
  one — the bundle would be incomplete exactly where §5.2's law says it exists.
...
**Spec.** §7.5, §12.7, §16, Appendix B

**Rationale.** §7.5's tiers completed: guards and `project` (unconditional per
boundary/frame) join the exactly-zero tier, handlers (episodic, only on firing)
join the tick tier's zero-by-idiom; publication is not a phase body — the §7.5
carve-out made structural; isolated invocation leaves buffers valid but
off-trajectory (re-`init!` to continue); §16's FlightCore allocation comparison
...
- *A no-publish advance mode on `step!`:* forks execution semantics for CI's
  benefit and breaks §8.3's publication-after-every-boundary property.
...
stated in §11.1, with qualified definition (`Flight.f(…) = …`, the `Base.show`
idiom) the recorded alternative for the extension-only periphery.

**Spec.** §11.1, §11.4, §16, Appendix C
...
message names the missing import); §16's exported-name audit widened to name
...
- *Relying on bare `using`:* silently defines unrelated same-named functions —
  ...
  namespace mistake as a modeling one, the §11.4 error-locality inversion
...
`TrimProblemInvalid`, discharging D-113's pre-staged clause. A worked
three-equation C172 cruise problem was added to §14.7.

**Rejected.**
- *Vector-returning residuals:* §14.8's failure reports name unbalanced
  equations — names must exist; packing is the service's job on both ends of
  the seam.
- *Passing `TrimParameters` as an argument:* §14.7 settled that the framework
  never sees it.
- *`tolerances` as a `trim!` keyword:* the §14.9 lift is field-by-field — a
...
unchanged, only at `Simulation` construction (D-048).

**Rejected.**
- *Declining as pure convenience:* the review's own tag — but D-049 made
...
**Spec.** §4.4, §11.2, §13.7, Appendix C

**Rationale.** `AbstractAtRoot` gains the remedy hint (wire a concrete
producer; in a rig, a stub child).

**Rejected.**
- *Relaxing abstract-at-root:* D-078 stands: staging cells, trace header and
  `probe_value` need concrete types at build.
- *A narrowing register at export:* new machinery, breaks the consumer-sourced
  slot-typing rule and §13.7's no-framework-support charter.
- *Leaving §13.7's "any component in isolation" claim unqualified:* false for
  exactly the class §4.4 exists to support.
...
**Spec.** §2.1, §4.1, §5.2, §7.1, §7.3, §8.4, §8.5, §9.7, §13.6, §14.3,
Appendix A, Appendix B

**Rationale.** §13.6's integration intermediates are renamed framework-owned
integrator buffers, never a component's `workspace` (§7.3's term is
...
observed" (D-082 restated, no semantic change).

**Rejected.**
- *Renaming "staging cell" itself:* entrenched through D-106–D-107, the
  walkthrough and the commit history; the compound is unambiguous once §4.1
  defines it.
- *Rewriting the log's historical D-004/D-013/D-024 to the new vocabulary:*
  amendments annotate, never rewrite.
- *Leaving §4.1's "throughout this document" rule broken by the spec's own
...
**recording registers → recorders** and the §10.6 **harness register → harness
cell**, reserving *register* for the idiom/mode sense.

**Spec.** §9.3, §9.6, §10.6, §11.2, §14, §14.5, §16
...
leaf walk's own verbs (`DeclarationOnWrongTier` untouched, D-112 intact — kills
the double-numbered collision inside §11.2). Guard condition → predicate: §14's
...
diagnostic kind named after the other sense was the worst collision; §9.6
...
§10.6 harness register → harness cell (§9.3's existing name). Deliberately
kept: *boundary zero* (a hyponym — it IS a step boundary, §14.5's whole point),
...
- *Compound-protection alone via glossary senses without renames:* leaves the
  §11.2 numbered-tier collision and the `KindUnreadable` self-collision
  standing.
...
**Position.** Appendix D Glossary is non-normative with a stated precedence
...
**Spec.** §3, §9.1, §9.3–§9.6, §11.1, §11.2, §11.6, §12.1, §13.7, §15.4–§15.5,
Appendix D

**Rationale.** §9.1 gains the **frame** anchor — one loop iteration (drain,
...
compounded. Drafting doubled as an audit and closed D-122's application
stragglers (component-sense "kind" residue in §3, §9.3–§9.6, §11.2/§11.6,
§12.1, §13.7, §15.4–§15.5; `KindMixed` → `ClassMixed`, completing the
...
- *A normative glossary:* every entry becomes a second source of truth to
  ...
  §11.1's condition for accepting redundancy cannot hold for it.
...
- *Defining *frame* in the glossary alone:* a non-normative entry cannot own a
  term; the §9.1 anchor gives it a normative home.

### D-124 — Widen §14.7's `reads` grammar to the full load-bearing selector set

**Status.** ratified

**Position.** §14.7's `reads` grammar widens to the full load-bearing set
(D-095's four plus D-125's `get_face`) with per-selector validation targets —
...
was a pre-D-095 straggler, not policy.

**Spec.** §14.4, §14.7, §14.8

**Rationale.** §14.4's source axis is restated as *table sources* (a boundary
snapshot, or the scratch tables a service evaluation instantiates, §14.8) vs.
store sources — the prior "boundary snapshot" wording did not cover trim, the
axis's primary load-bearing client; D-098's enforcement (snapshot-bound store
selector = attach-time `ReadBindingUnresolved`) is unchanged.

**Rejected.**
- *Keeping §14.7's two-selector grammar as a deliberate narrowing:* D-095
...
through export chains exactly as §14.9's mounting resolves slot faces (the read
side mirroring the write side), so an equilibrium equation crossing a generic
seam binds the curated face register. This amends D-095: the four-selector set
was inherited from D-083 and predates `get_face`'s spelling (D-095), so this is
completion, not reversal.

**Spec.** §6.1, §7.4, §14.4, §14.7, §14.9, §16

**Rationale.** Companion idiom recorded beside §14.7's `reads` bullet: a
derivative a service must read across a contract boundary is published as an
ordinary output port computed in `h_xu` (§7.4 step 2's one-line binding, made
...
derivatives" the library-convention heuristic deferred to the §16 migration
outline. Grounding: the C172's engine-speed equilibrium
(`ẋ.systems.pwp.engine.ω`) is unspellable under D-083/D-095 alone — every level
of the real aircraft tree is a generic seam.

**Rejected.**
- *Load-bearing deep paths past generic seams / instance-resolved `reads`:*
  dissolves D-083's register split exactly where it pays: a same-path/same-type
  substitution silently changes the committed trim point — the drift `get_face`
  exists to shield; breaks §14.9's relocatability; §6.1 forbids the traversal
  even where the concrete instantiation would resolve it, and D-061 fixed
...
- *A face-level `ẋ` spelling:* reintroduces a zero-promises register at the
  ...
  boundary-consistent (D-098).
- *Auto-publishing every state derivative:* contract bloat, and routes
  integrator scratch into snapshots against D-098's source axis.
...
**Spec.** §14.5, §14.8, §14.10, Appendix C

**Rationale.** A commit-fired handler moves the committed stores off the
reported solution — which `capture`-defaulted `linearize` (§14.10) then reads —
and §14.8's doctrine is the elimination of warn-but-assign, so a wanted failure
...
- *Suppressing or rolling back commit-fired handlers:* boundary zero is an
  ordinary boundary by §14.5's design — D-100's visibility philosophy: surface,
  don't special-case.
...
captured with the stores. §10.7's up-front pass treats it in two registers:
`Δt_base`/`h`/`n`/algorithm compared against the target's deployment binding
(widened to six by D-133: `localization_tol` and `event_budget` join both the
...
at replay as at `run!`, D-091).

**Spec.** §9.5, §10.7, §13.5, Appendix B, Appendix C

**Rationale.** Defined as the artifact "run metadata" already named three times
without a home (§13.5 ×2, Appendix B; D-091's obligation, now discharged).

**Rejected.**
- *Validating the whole block uniformly:* freezes `t_end`/`stop_on`, which
  §10.7 explicitly leaves overridable, and a recorded `stop_on` termination
  reproduces itself anyway.
- *An overridable deployment check to allow what-if-with-different-`h`:* not a
  what-if under §10.7's own definition — the register promises the recorded
...
halts at a frame top (both §10.7 promises preserved — ends `initialized`, the
...
at which the failing frame began — making §13.4's recipe total and exact:
...
**Spec.** §8.4, §10.6, §10.7, §13.4, Appendix C

**Rationale.** The boundary counter stays the reporting index (§8.4's
...
- *A frame-ordinal `to_frame` keyword beside `to_boundary`:* a second
  ...
  closes.
- *Halting replay mid-frame at `t*` boundaries:* breaks §10.6's whole-frame
...
(§9.3, unchanged), the read side speaks structure in the inspection register
and contract wherever meaning must outlive the build: integration bindings
(§9.2's `get_face`, D-095) and load-bearing service reads (§14.4, D-125).

**Spec.** §9.2, §9.3, §11.6, §14.4, Appendix D

**Rationale.** Swept: §11.6's rule sentence, §9.3's parenthetical, §9.2's
subheading (now "snapshot bindings") and the glossary face entry — all four
predated D-095's `get_face` and stated read = slash-paths-only, forbidding the
binding style §9.2 itself recommends.
...
- *Keeping the read/write phrasing with a `get_face` carve-out footnote:* the
  ...
  freedom is register choice (D-083); a footnoted exception would misstate the
...
register-scoped — D-083's line carried into resolution: *structural* (wiring,
...
authoring-level, §14.2 — absolute paths are a compiled derivative — and the
mount prefix is checked by the mount itself, §14.9's contract validation);
...
register a client resolves under is internal, never user-facing API (§14.4's
`apply!`-register status). This amends D-061.

**Spec.** §13.3, §14.2, §14.4, §14.9

**Rationale.** Not D-061's rejected instance-only walk: the declared-type walk
stays the primitive's semantics and stays enforcing where promises depend on
it.

**Rejected.**
- *A single unconditional strict rule:* as written it made D-083's blessed
  ...
  §14.9's mounting on compiled absolute paths.
- *A public register parameter on `resolve`:* client policy is
  framework-internal; §14.4 precedent.
- *Permissive resolution for load-bearing clients at the compiled path:* erases
  the locality law exactly where substitution safety pays — D-125's rejected
  deep-path reads.
...
consistency sweep — stragglers of the restructure and of the D-121/D-122
...
**Spec.** §5.3, §6.2, §7, §7.1, §7.5, §9.4, §9.7, §10, §10.6, §12.5, §13.4,
§14, §14.6, §15.4, §16, Appendix B, D.3, D.6, D.8

**Rationale.** §16's GUI bullet lists orphan display among the settled
semantics, not the deleted claim-transition policy (D-047, D-106); §9.7's
closing constraint bakes port resolution *and* the liveness verdict at run
start, matching its own mid-text, §15.4 and D.6; three D-122 stragglers renamed
(§12.5's probe-derived *predicate* form, Appendix B's bundle footnote *function
family*, Appendix B's `Simulation` entry *unattended run*); two D-121 slot
slips reworded (§14.6 "the one initialized datum without declared defaults",
§9.4's scatter "position → slot cell"), the claims themselves intact; chapter
9's opener roadmaps only chapter 9, leaving §10's own opener the sole roadmap
for chapter 10; `project`'s two schedule positions cited to §5.3, where the
rule actually lives, from §7.5, §12.5 and D.3, with a one-clause back-reference
added at §7.1's write-back bullet; §10.6 says *five* states, agreeing with its
own enumeration and with §13.4, §14's legality tables and D.8; and the two
walkthroughs' six self-references, mis-resolved to unrelated spec chapters by
the phase-4 linkify pass, now point at their own headings (the stray §6.2,
which matches no walkthrough heading, retargeted to the walkthrough's §6, token
...
- *Making the self-vs-spec case a hard checker error:*
  `event_visibility_walkthrough` legitimately cites the spec's §7 while owning
  a §7 of its own — the ambiguity is real, only the author can settle it, and
  the check would fail on correct text.
- *§10.6's "four phases, the last terminal in two flavours" grouping:*
  defensible but it re-describes what §13.4, §14 and D.8 all count as five
  statuses.
...
§10.4(1)'s initiation sources: the run completes the current boundary,
publishes the final snapshot, takes the ordinary graceful tail and ends
`stopped` — boundary-consistent, serviceable by the §14 services, resumable by
the next `run!`.

**Spec.** §9.6, §10.4(1), §10.4(3), §10.4(5), §13.4, §13.5, §14, §14.8,
Appendix C
...
clauses fall out: §13.4's catch site never wraps an `InterruptException` in
`StepError` (routes it to the stop path; unreachable in practice under masking,
kept defensively), and §9.6's device wrapper never turns one into `DeviceCrash`
...
§10.4(5)'s abandonment path taken at once, devices still named — and the run
still ends `stopped`; escalation shortens the tail, never reclassifies the run,
and cannot repair §10.4(5)'s honest asymmetry (nothing can abandon the task
...
source (§13.5), distinguishing it from `t_end`, `stop_on` and a
programmatic/GUI stop, with no new diagnostic kind; `UnboundedRun` (Appendix C)
...
happened (§14.8). Precedent correction, recorded because it is easy to miscite:
...
§10.4(3)'s `unblock!`, not of this path; FlightCore has no
...
`localization_tol`, a *relative* bracket-width convergence test for §8.4's
...
grid-independent, hence outside §8.5's harmonic-grid check, and **recorded** in
the trace header's deployment block, which takes replay's compared set from
four to six (§10.7). The four **operational** ones get spec-fixed defaults in
their owning sections and no user knobs: shutdown join timeout 5 s (§10.4(5));
debt forgiveness at five frames' worth of budget, `5·h/p` (§8.7, replacing "a
few frames' worth"); heartbeat staleness 2 s (§10.2); `margin` = 2 ms (§8.7's
own arithmetic), replacing Appendix B's `margin = <default>` placeholder.
§13.2's per-device warning rate limit alone is **deferred** to the
diagnostic-stream decision [closed by D-136: the bound is the diagnostic cell's
ring, not a constant]. Finally `t_end`'s landing rule, stated once in §10.4(1):
...
carrying the actual final `t` (§13.5). This amends D-127.

**Spec.** §8.4, §8.5, §8.7, §9.5, §10.2, §10.4, §10.4(1), §10.4(5), §10.6,
§10.7, §12.1, §13.2, §13.5, Appendix B, Appendix C, Appendix D
...
§8.4's "the run replays identically" is an empty promise unless what the
...
illustration is exactly this value. `t_end`'s landing rule follows §10.6's
...
- *Leaving the two semantic constants as implementation constants:* silently
  breaks §8.4's replays-identically promise — a trajectory-determining fact
  must be deployed and recorded, not buried in the localizer.
- *User knobs for the operational four:* ownership was the gap, not
  configurability — and §8.7 already settled `margin` as the single pacing
...
- *A short final step landing exactly on `t_end`:* grid integrity `tₖ = t₀ +
  k·h` forbids it, §8.4 — the bookkeeping hazard that rule kills.
...
(§9.6's method-presence idiom).

**Spec.** §9.2, §9.3, §9.4, §9.6, §9.7, §10.3, Appendices B/C/D

**Rationale.** Completes D-044's partition. `interactive` defined ⇒ derived
write side: no claims staked, and §9.4's positional staging shape compiled over
...
`latest` per render, §10.3 — with an ad-hoc render-time read set over the whole
snapshot in the inspection register's shape, §9.2, so the compiled gather has
...
rostered" (D-109's idempotency, now stated against the class rather than the
type). Two non-reopenings are on record: §9.6's rejected `sides(b)` trait is
...
D-044's rejection of unclaimed-register opportunism "for any device" stands
...
- *A shipped concrete `InteractiveBinding` type:* edges toward the abstract
  binding-type taxonomy §9.6 already rejected, and abandons the method-presence
...
- *Leaving the class undeclared with the GUI special-cased:* the status quo
  under review — §9.3's "the GUI is not an exception but the resident of the
...
**Position.** Round-4 gaps finding 5, amending D-052: the activation cache is
...
since an activation is a pure function of the build and the scalar (§12.4's own
words).

**Spec.** §7.5, §9.1, §12.2, §12.4, §14.8
...
— §14.8's per-invocation rule promoted from a trim-local statement to the
general one, D-070 being the decision that forced it (iterating on shared
...
O(model size) and trivial against the solve it feeds, and §7.5's
zero-allocation invariant being scoped to the stepping loop (the services were
always allocation-tolerant, §14.8). Two consequences stated where they belong:
the `Build` is an **immutable artifact that may back any number of
`Simulation`s, concurrently** (§12.2 — true by construction once buffers are
single-owner, which is what §12.2's `Simulation(build::Build; …)` factorization
plus §9.1's inline-`run!` parallel sweeps were already inviting), and **lazy
...
duplicated work). Recommended idiom for the §9.1 parallel-sweep register,
...
- *Cached shared buffers:* D-070's aliasing — warn-but-assign reborn — and it
  ...
  forbids.
- *Per-`Simulation` activation caches:* defeats the sharing §12.2's artifact
...
**Position.** Round-4 gaps finding 6, closing D-133's §13.2 deferral: the
...
itself, single-writer by the same ownership argument as §9.4's staging cells
...
**Spec.** §7.5, §8.7, §9.1, §9.2, §9.4, §9.6, §9.8, §10.2, §10.4, §13.2, §13.5,
Appendices C/D
...
retained — which homes §13.2's "rate-limited wherever its source can repeat" as
a structural property of the channel and discharges D-133's deferral with no
constant to tune. The loop takes each cell with `atomicswap` at the **frame-top
drain**, §9.4's own point, swapping in a **shared empty sentinel** so a quiet
...
primitives (§9.6), acquire-loaded at the drain, §10.2's 2 s staleness read
...
each status) and `heartbeat`, beside §8.7's pacer diagnostics — §9.2's binding
...
(§10.4(1), §13.5), so an unattended run's complete diagnostic account survives
...
per-boundary snapshot allocation §9.2 already accepted — which requires the
per-kind counters to be a **fixed-shape isbits record, never a `Dict`**,
licensed by Appendix C's closed kind set — while a noisy frame allocates its
...
unwritable thereafter), the same GC-over-reuse trade §9.2 made when it rejected
preallocated snapshot buffers; worst case per writer per boundary is therefore
one ring of 16, everything past it an integer increment, and §7.5's scoped
...
- *A shared queue or buffer under a lock:* cross-task contention on the loop's
  ...
  shared mutable state §9.1's two rules exist to eliminate, reintroduced for
...
- *A status referencing the live accumulator:* the cheapest thing to write and
  a direct violation of §9.2's binding rule — a reader walking a structure its
  writer is still mutating.
- *Ring reuse by double-buffering:* reintroduces exactly the reader-liveness
  proof the GC provides for free — §9.2's own argument against preallocated
...
- *A per-device rate-limit constant, D-133's deferred option:* the structure
...
**Spec.** §4.1, §4.4, §7.5, §8.7, §9.2, §9.5, §9.8, §10.4, §10.7, §13.5,
Appendices B/D

**Rationale.** **A count, not a memory budget**: snapshots are immutable object
graphs with internal sharing (§4.1, and §4.4 field handles riding as references
to build-time-frozen data, §7.5), so byte accounting over them is fuzzy and
...
density, which is D-038's division of labor carried through (the log's chief
consumer is the post-run plot of a session as a whole; full density over any
*segment* of interest is what replay from the trace recovers, §9.5, §10.7). The
licence is D-038's own asymmetry, a fortiori: a thinned log costs resolution in
a *view*, never a record, whereas D-029's refusal to thin the trace stands
untouched. Following D-135's precedent, **the guarantees are normative and the
...
(§8.7) — exponentially rare, the wall-clock gap between generations doubling,
...
framework side of §7.5's scope, publication staying wait-free with readers
never blocking. **The boundary-zero (§14.5) and terminal (§10.4(1), §13.5)
...
endpoints and, via the terminal status's final cumulative counters (§9.8,
D-136), its complete diagnostic account always survive. Compositions stated
once: `totals` monotonicity across logged snapshots (§9.8) is unaffected,
...
recorded nor compared by replay (D-127, D-133's compared set untouched). §7.5's
...
- *An unbounded default, the status quo under review:* gigabytes per hour at
  C172X scale and 50 Hz in the configuration Appendix B calls the honest
...
of the §10.4 protocol**, taken at the top of a run — once per roster entry, in
...
**Spec.** §9.1, §9.3, §9.6, §9.8, §10.2, §10.4, §13.5, §14, §14.6, Appendices
A/B/C/D

**Rationale.** Release is therefore **unconditional**, which is what makes
§9.6's "guaranteed on every exit path" and §10.4's "`shutdown!` has released
each device's OS resources" true of the one path outside the wrapper, and it
carries a **new taught obligation** (§9.6, Appendix A): `shutdown!` must
...
already fits an init-time failure, Appendix C's gloss merely naming the site. A
...
against §10.2's threshold from the first frame (§9.8, D-136) — and its **claims
persist to run end**: §9.3's death-is-not-detach disposition applied one step
earlier than §10.4(6)'s, the orphaned slots holding their initial values,
well-defined by §14.6's slot totality where a (6) orphan holds a last drained
...
zero**, so the run reuses §13.5's existing terminal-at-`t₀` path — remaining
...
through the ordinary §10.4 tail and the termination record names the source.
**Topology is derived after initialization** (§9.1): a failed
...
(Appendix B, D-134's sugar unchanged); Appendix D gains the glossary entry.
...
- *Short-circuit teardown on an aborting init failure, ending the run without
  publishing:* invents a second exit path beside §10.4's one tail and yields a
  `stopped` sim with no terminal snapshot — unserviceable by §14, with nothing
...
- *A dedicated init-failure diagnostic kind:* `DeviceCrash`'s payload and
  disposition already fit, and Appendix C's closed set pays for every addition.
...
- *`should_abort` defaulting to `true`:* a dead peripheral kills a session —
  the opposite of §10.4(6)'s stated intent.
...
output stage a **one-line call to it and never the reverse** — stated in §4.4
as a **shipped component's obligation** (Appendix A taught contract, Appendix D
...
silent-drift class §5.3 exists to kill.

**Spec.** §4.4, §5.3, §12.6, §14.1, §14.2, §14.7, §14.9, §16, Appendices A/D
...
load, acceptable because condition authoring is design-time code. §14.1's
...
new mechanism. §14.1 also records the **environment-free fallback** through the
...
walkthrough, then §16). **Trim problems never author the environment** (§14.7,
§14.9): handles arrive through the user parameter record, the problem
...
naming a wired input fails resolution by name, correctly (§14.9), so a problem
that wrote an environment face would apply only to rigs where that face happens
to be unconnected. §14.9's "the aircraft is never literally the root" is
...
linearization's input surface and the trace header already speak. §12.6's
...
respelled as a field handle (built at value level per §4.4, or held as a rig
...
§16's migration outline**: under elimination a params-vs-world handle mismatch
...
- *Widening the condition signature to `condition(d, baseline)` so the math
  ...
  baseline it is applied against — the hidden-input shape §14.1's overlay-base
  decision kills, reintroduced one level up — and it breaks the pure condition
  algebra that makes §14.9's `at`-lifting five lines.
- *A pre-application or partial sweep making swept outputs readable at
  condition time:* reaffirms §14.2's by-name rejection of init as a third
...
Stated to forestall the reading: none of this reopens D-008's rejected resource
...
**Position.** Round-4 dry-run finding 4 (§11.3 and §4.3 unchanged): §5.4's
escape hatch becomes a **three-rung remedy ladder** for artificial loops.

**Spec.** §4.3, §5.4, §5.6, §11.3, §16

**Rationale.** (i) The **two-stage split** dissolves most of the class —
§15.1's `VehicleDynamics` instance is the canonical dissolution. (ii)
**Contract re-factoring**, the new middle rung: when §5.6's tracer says
...
its cost is now **stated where it bites**: §11.3's visibility is binary, so
every intermediate shared across the new boundary becomes `output_types` —
public, connectable, substitution-relevant. The cap is §4.3's granularity
...
and honest one. §5.6's hint offers **both exits** ("split this component, or
narrow the neighbor's contract"), continuous members only as before, and the
classifier machinery is untouched: it already knows which hop died. §5.4's
"rare" is thereby **earned** rather than asserted — the split is rare *because*
rungs (i) and (ii) absorb the common shapes. §16 records the strut/steering
pair as the contract-re-factoring worked instance, beside §15.1's dissolving
...
**aircraft-library migration call living in §16**, not framework vocabulary.

**Rejected.**
- *A visibility register for split-orphaned intermediates (`unlisted`,
  `Private(T)`):* an explicit non-reopening of D-034 and D-055 — the promotion
...
- *Leaving §5.4 split-only:* the first real migration instance shows the
...
- *Framework-side auto-splitting or auto-narrowing:* the classifier's trace
  speaks only for the branch taken at the probe point (D-012's diagnostic-only
  doctrine), so remedy selection is the author's — and both §5.4 remedies
...
entry is marked explicitly discrete-tier, and §15.2's gear sentence is extended
to say why. §3.1, §2.1 and §13.7 are unchanged. Even a commanded continuous
...
implemented internally as an ordinary guard/handler event (the Appendix A
contract's worked instance). The gear wires `strut.wow → frc.reset` (the
touchdown edge).

**Spec.** §2.1, §3.1, §3.3, §8.6, §11.5, §13.7, §15.2, §16, Appendix A
...
SUNDIALS stop-and-`ReInit`, DiffEq.jl callbacks), which is also why §3.1's
immutable stage views are enforcement of mathematics rather than a
discretionary restriction. No stale-output hazard rides along: §8.6 re-sweeps
...
a single migration consumer, which is how §13.7's grow-by-migration-demand rule
...
their `f_init!` gain writes becoming construction-time parameters (D-089). The
PI law is shared as plain pure functions called by the block's stages (D-139's
...
- *Simulink's full policy menu (rising/falling/either/level triggers plus an
  external-IC port):* one policy suffices, with §13.7's Bool gates covering
...
- *Relaxing §3.1's immutable views* so stage methods could write `x`: forbidden
...
instance (§11.5), not an opaque toggle.
...
throw is a `UserCodeFraming`-wrapped build failure (§13.1's fail-fast user-code
...
the run ends `errored` (§13.4) — §13.5 already rules that exceptions from model
...
**Spec.** §12.3, §13.1, §13.4, §13.5, Appendix A

**Rationale.** §13.1, §13.4 and §13.5 are otherwise unchanged by this finding.
...
`Bool` output face plus `stop_on`, §13.5's existing machinery, so §12.3 only
...
that the function is partial. The statement lands as one §12.3 paragraph,
...
Appendix A recall line. D-050's probe-everything scope stays unnarrowed by this
finding.
...
- *Probe-time exception downgrading (catch user-code throws while probing and
  ...
  throw to the first real step as a `StepError`, and breaks §13.1's clean
...
- *Doing nothing beyond §13.5's termination mapping:* it covers the termination
...
- *Narrowing D-050's probe-everything scope to spare the offending functions:*
...
§6.2 gains the spelling for a zero-contributor aggregate. Contract: an ordinary
...
clarifications ride with it: (i) this is not §6.1's banned default in component
...
a configuration wanting an externally settable source uses a root slot (§9.3),
the boundary that keeps the block from drifting into a back-door input default.
Tier: stateless continuous, so §13.7's existing tier-transparency argument (a
...
**Spec.** §6.1, §6.2, §9.3, §13.7, Appendix D

**Rationale.** D-037's retirement and §6.1's ban are preserved, not reopened,
...
§6.1 forbids unconnected inputs and silent defaults, a zero-arity `SumJunction`
would need the identity element D-037 retired on purpose, and the library had
...
zero-edit convenience is the hazard (D-037's own words). It also subsumes the
rig stub: D-120's concrete `SampleTerrainField` provider wired to an abstract
...
wherever the double must compute something. Admission honors §13.7's strictly
from demonstrated need charter: the dry run demonstrated the need twice, at the
zero-contributor totals and at the rig. Appendix D gains a constant source
entry.

**Rejected.**
- *A zero-arity summing junction with an identity element:* D-037's retirement
...
- *Consumer-side optional inputs or unwired defaults:* §6.1's ban — the silence
  is the hazard, the same property D-037 rejected in the tree walk.
...
unchanged. §9.6's `faces(b)` claim-set declaration is deliberately untouched:
...
application of the four-register naming convention now recorded as §16's
...
`get_slot`, `get_face` (§14.4); (3) lifecycle and mutating actions are verbs,
`!` when they mutate — `build`, `run!`, `step!`, `replay!`, `init!`, `attach!`,
`detach!`, `shutdown!`, `apply!`; (4) build primitives (§13.3,
...
residuals are flagged and deferred to the §16 audit, not renamed now:
...
**Spec.** §9.6, §11.6, §11.8, §12.1, §13.3, §13.7, §14.4, §15.4, §16, Appendix
B (swept)
...
§11.8's own words ("the helper exists for the pass-through case, where an
assembly hands a child's unfed requirements up one level"), reads as an
operation rather than a declaration, and stays accurate if §11.8's guarded
...
- *Renaming `input_faces`/`output_faces` or `workspace` in the same pass:*
  ...
  deciding them now would settle by anticipation what §16 exists to settle
...
assembly's `connections`. No new vocabulary and no framework change: §11.8
...
§11.5's blessing of declaration bodies as ordinary code doing the work.

**Spec.** §11.1, §11.4, §11.5, §11.8, §14.6

**Rationale.** D-043 is reaffirmed, not reopened, by this finding, which closes
the round-4 dry-run report. The drift is caught loudly — the design working —
but the duplication is §11.1's and D-039's "structure kept in two artifacts"
...
for conditions to cover (§14.6) — what the idiom preserves and the rejected
...
- *A framework helper deriving `except` from `connections`* (`except = fed(s,
  "aero")` reading the assembly's own wire list): auto-bubbling on D-043's
  ...
  shape, at worst not at all once a GUI or condition writes it — §11.4's
  walkthrough 2 inverted; the single source must be authored data, never
  inferred structure.
- *Wildcard-export vocabulary:* D-043 stands — ordinary code suffices, and this
...
detection-by-method-presence unchanged. Rows ≤ D-145 are historical and stay
unamended. `claims(b)`: what the declaration does is stake the claim set (§9.3)
— the enumerated faces become the device's claimed write surface — so the name
plugs straight into §9.3's central claim vocabulary (claim set, claim
partition, `ClaimedFaceEntry`, claims orphaned to run end) and retires the last
method-name use of the spec's most polysemous noun, "face" (D-144 dissolved the
other one, leaving this the sole survivor). `reads(b)`: the declaration
establishes the binding's declared read set — a tuple of §14.4 selectors,
validated at attach and compiled to one per-boundary gather whose labeled
NamedTuple `map_output` receives — and §14.7's `TrimProblem` already carries
...
selectors, store selectors, path selectors, §14.10's tap selectors, "§14.4
selectors"); only the binding method renames, and `reads(b)` still returns
§14.4 selectors.

**Spec.** §9.3, §9.6, §14.4, §14.7, §14.10, §16, Appendix B, Appendix C,
Appendix D (swept)

**Rationale.** This is the first application of D-144's four-register
...
load-bearing, a device read binding diagnostic) is no obstacle — D-130 made the
...
- *Keeping `selectors(b)`:* defensible status quo — content-accurate and
  ...
  §14.7 unification a shared `reads` buys.
- *`taps(b)`:* spoken for by §14.10's linearization vocabulary — the
...
guard probes run, so the mid-step ZOH holds by construction (D-019's ZOH
...
the pre-existing §8.5 mechanism, now explicitly scoped to boundaries, the only
place anything discrete ever happens (§8.6's macro-sequence is its sole
...
`h_xu` walk. Phase-body surface, amending D-116: the two sweep bodies gain an
...
previous frame's due set — and everything at boundary zero, which is §14.5's
existing rule (`t₀` is a grid point of every divisor, and no earlier tick
exists for a ZOH to hold) rather than a new one.

**Spec.** §5.3, §8.3, §8.4, §8.5, §8.6, §12.7, §14.5, Appendix D

**Rationale.** The split also recovers an implicit commitment: §12.7's
...
- *Superseded position — counter-modulo gating as the sole mechanism (the prior
  ...
  §8.5's own rejected alternative names, and undiagnosable: such a run is as
...
- *A single parameterized body taking a sentinel "no ticks due" index* on
  ...
  RHS-side body, falsifying §12.7's recompilation-granularity claim; and it
...
- *Reading the tick index out of simulation state inside an unparameterized
  `sweep_hxu()`:* D-116's surface read literally — the accessor can then no
...
decision-dependent leaf is `Dual`-typed in the shape (§14.7), which is what
...
**Spec.** §12.5, §14.3, §14.4, §14.7, §14.10

**Rationale.** A round-5 kernel dry-run finding (finding 2; §14.10 unchanged),
reaffirming D-066's resolution-time bake with its selection rule refined. Leaf
types are **shape facts** — §14.4 already fixes that the tree type carries the
...
D-066's timing exactly. **No new machinery, no new user surface**: under the
...
**§14.10 needs no amendment** — a linearization operating-point condition is
...
intact: §12.5's nominal exact-match doctrine for table cells is untouched (at
...
- *A uniform zero-partial embedding for every non-nominal condition write:* the
  prior literal reading of §14.3, and the finding — trim's decision-descended
...
- *Per-write runtime convert decisions:* D-066's rejected alternative, still
...
**Spec.** §9.3, §12.3, §14.6, §14.8, Appendix B, Appendix D.8

**Rationale.** A round-5 kernel dry-run finding (finding 4; §9.3, §12.3,
Appendices B and D.8 swept), amending D-068's two-site placement. Coverage is a
...
before a write. §14.8's raw scratch instantiation is thereby sound **because**
...
or is needed, and §14.8's no-throw doctrine holds without exception. §14.6's
no-third-branch ban on `probe_value` is untouched.

**Rejected.**
- *`probe_value`-filling the scratch slots:* the §14.6 ban stands — it would
...
backend-independently** — `converged ⟺ all(abs.(rᵢ) .≤ tolᵢ)`, §14.7's
...
**Spec.** §14.7, §14.8, Appendix B

**Rationale.** A round-5 kernel dry-run finding (finding 6), extending — not
amending — D-069's service-side squaring. **The tolerance translation into each
...
- *Backend-owned verdict:* the prior literal reading of the seam sentence,
  ...
  §14.7 itself criticizes FlightCore for, and the finding's divergence follows
...
where the existing test folds — so §12.5 keeps one whole-type `isa` (never
per-field checks) and §7.5's canary verifies the fold empirically.

**Spec.** §7.5, §12.5, §14.7, §14.8, Appendices B/C/D (as touched)

**Rationale.** A round-5 kernel dry-run finding (findings 7 and 8): D-053's
fold economics preserved, D-118's setup placement kept and its key-set check
...
- *Per-field checks:* D-053's rejection stands — canonicalize-then-single-test
...
**Rationale.** A round-5 kernel dry-run finding (finding 5), amending D-016.
Auto-published cells belong to no stage (§12.5), so D-016's stages-only
...
- *Stage-products-only handler-phase `y` with the hole documented:* preserves
  D-016's letter but silently abandons the `y = h(x)` coherence invariant that
...
the prior (D-082), a `fired` flag (the once-per-event rule's register), and a
...
**Spec.** §8.6, §9.7, §9.8, Appendix C

**Rationale.** A round-5 kernel dry-run finding (finding 9), completing D-082's
...
captured, reset by warm restart (D-082's doctrine extended); cost budgeted
...
- *The warn-on-holding-after-fired rule:* the only one implementable from the
  ...
  per-boundary warning stream consuming the 16-entry ring §9.8 budgets for real
  diagnostics — the anti-diagnostic pattern §9.7 names.
...
- *Prior = final quiescent sample unconditionally:* D-082 as written — swallows
...
evaluated every round for D-153's samples; the firing step is what's capped;
...
(f.5's incoherence dissolved by deletion where D-152 dissolved it by addition),
...
supersedes D-016's per-event re-decode, its D-152 auto-publish amendment, and
D-100's pre-materialization mechanism (D-100's round-start-`u` *semantics*
...
**Rejected.**
- *Per-event re-decode + frozen `u`:* D-016/D-100/D-152 as landed — two
  freshness classes needed D-152's repair, and delivery needed f.10's two-pass
...
- *Retained guard-gathers as the carrier:* this round's interim proposal —
  ...
  freeze at all.
- *Live-table reads under canonical order:* §8.6's standing rejection —
...
residual sets: the solved-point residuals (D-150's verdict numbers, unchanged,
...
`TrimCommitResiduals` (Appendix C: offending residual names, committed values,
tolerances) — flag-when-it-occurs applied to the moved point.

**Spec.** §14.5, §14.8, Appendix C
...
evaluates the residuals once at the baseline and D-150's box test decides
...
invoked with a zero-length buffer**, no backend faces N = 0, completing §8.1's
...
(§11.4's error-locality inversion, otherwise reintroduced at runtime); **`ẋ`
...
and `ẋ` buffers are integrator scratch, not boundary-consistent (D-098's
register).
...
- *Adding an `ẋ` sweep:* redundant by the contamination argument, and it would
  bless reading integrator scratch against D-098's source axis.
...
`tol` in `tolerances`' order, data a backend may stop on (D-150's per-register
translation), decisive of nothing; `status::Symbol` a deliberately **open** set
recorded verbatim (D-150 demoted status to diagnostic — a closed enum would
launder foreign solver vocabularies back into per-backend meaning);
`nevals`/`niters` diagnostic counts; the name `solve` rides the §16 naming
audit like all API spellings.

**Spec.** §14.8, §16

**Rationale.** A round-5 kernel dry-run finding (finding 14), making D-118's
one-implementation-target real and upholding D-150/D-151.

**Rejected.**
- *Closure-style `d -> (r, J)` returns:* allocates every iteration.
- *Jacobian via a second method or a boolean flag:* doubles D-118's single
...
- *Bounds as a vector of tuples:* both shipped backend families take two
  vectors.
- *A closed status enum:* re-imports the per-backend `converged` meanings D-150
  removed.
...
drawn from the report the call returns; §13.2's two-stream partition stands — a
service warning is not a stream but a synchronous per-call annotation.

**Spec.** §13.2, Appendix C

**Rationale.** A round-5 landing-audit finding (finding 1), completing
D-126/D-155 and superseding D-092's five-value count to six.

**Rejected.**
- *Demoting `TrimCommitEvents`/`TrimCommitResiduals` to report-only data:*
  silently reverses D-126/D-155's made-visible-rather-than-silent doctrine and
...
runtime warnings into the device's diagnostic cell (§9.6, §9.8) and mutating
actions carry the bang, beside its already-banged contract neighbours `stage!`
and `mark_dead!`.

**Spec.** §9.6, §9.8

**Rationale.** A round-5 vocabulary finding (finding 2.1), per D-144's register
(3).

**Rejected.**
- *Keeping the bare spelling:* a mutating action in noun dress — the register
  violation D-144/D-146 exist to retire.
...
sharpest case (one name, two senses — the D-122/D-144 pattern), and an explicit
...
flattened by §7.1's leaf walk into one contiguous buffer per element type,
build-time offsets carried in entry fields, the port type in the address
token's parameter** (§12.7 amended).
...
rather than model size — which demotes §12.7's mitigation ladder to optional —
...
evaluator, a hand-rolled per-component check, today's FlightCore path in §16's
comparison) does not, and along a dependency chain the last-ulp difference
accumulates.

**Spec.** §6.2, §7.5, §12.7, §16
...
3945596. Two consequences past test hygiene: §12.7's chunk size remains a
genuine implementation freedom only while no test asserts bit-exactness across
chunk sizes, and §16's FlightCore value comparison is a tolerance comparison by
construction, not by concession. Determinism is untouched — within one build
the schedule is fixed and §6.2 made every sum an ordered junction entry, so a
run reproduces itself bit-for-bit.

**Rejected.**
- *Bit-exact assertions under a `muladd`-free or pinned-math build:* buys a
  brittle equality by giving up the codegen §7.5's invariant depends on.
...
increment-2 finding, extending §11.1.
...
local-scope sibling of §11.1's `using Flight` trap, and worse-behaved: the
...
**Spec.** §5.2, §5.4, §7.5, §9.2, §11.3, §12.5, §12.7, §13.2, §14.4
...
producer ⇒ no `w` key ⇒ the §13.2 framing diagnostic on a consumer that
destructures it).
...
the nominal probe observed"), complementing §7.5's canary (detection) with
localization.
...
candidates = `output_types`; `get_local` retired from the §14.4 selector family
and §9.2's binding registers — the inspection path for intermediates is
*promote to output* (FlightCore precedent: intermediates were only ever
inspected by inclusion in the `Model` output); local cells leave the §12.7
store; §11.3 visibility becomes: declared in `output_types` = public, returned
...
- *Walking the nominal-observed type to synthesize non-nominal expectations:*
  ...
  one misses.
- *`local_types` status quo (D-034/D-055):* declaration + table cells +
...
- *Persisted `w` slots / table cells for `w`:* a cell needs a fixed
  ...
  makes probe-observation sound here where D-034/D-055 rightly rejected it for
...
shape, §11.5), so the whole-signature forgotten-`T` variant — the nastiest
member of the class in the D-033 world, where the plain form *was* the tier
marker — is extinct by construction.

**Spec.** §7.1, §7.3, §11.1, §11.2, §11.5, §12.4, §12.5, §14.10, Appendix D
...
delivering §14.10's recorded door** (declare `Float64`, strip with `value()` in
the stage; the freeze is declared and conformance-checked); `Int`/`Bool`/ enum
and reference-typed leaves as before.

§12.5's embed-accept is re-keyed on declared-`T` leaves: a `T` leaf accepts
...
§11.1's position (optional, on top, never required).
...
by rule — §7.1 forces every state leaf to follow the activation scalar;
...
the scalar** (D-077's `workspace(c, T)` precedent).

Reversal grounds vs D-079, recorded so it is never re-litigated from a worse
...
walk rule in their head), and two of the three grounds that decided D-079 had
...
made the original `T`-form look fragile. D-079's two conceded losses
("schema-visible participation, whole-leaf stripping detection — tooling
niceties") are re-valued as the point.

**Rejected.**
- *Plain declaration + leaf walk on the output side (D-079's design):*
...
- *Two-argument `init_x` by evaluation:* the `T` records no choice — §7.1
...
- *Two-argument `init_m`/`init_z`:* nothing could ever follow `T`; the
  signature would invite exactly the misplacement §11.2's didactic errors exist
  to catch.
...
**Spec.** §6.1, §11.2, §11.5, §13, §14.10, Appendix D
...
Escape from D-033's rejection, recorded: the permissive reading predicts
...
- *The predictive reading ("this is what will arrive") and the envelope reading
  ("I promise to promote"), D-033 upheld:* the predictive reading remains
...
- *Symmetric `T`-forms on the discrete tier:* both directions
  ...
  and discrete reads never see one; if §14.10's sampled-data door opens, the
...
AD-opaque table is §11.2's FFI door in use — and the meet is the only
...
§12.5) while a `Dual` arriving at a `Float64` entry is exactly what that entry
forbids; it is the mirror of §6.1's producer-side walk-compatibility clause.
...
Closes the fan-out gap left open by D-167.
...
- *Join semantics (the slot walks if any consumer tolerates):* unsound in the
  ...
  cannot take them, the exact failure §6.1's clause exists to prevent, moved
  from build time to inside user math.
- *Per-tolerance-class slot cells (one cell per class):* duplicates the slot,
  defeating §9.3's one-cell-per-slot staging, the trace header's single type
...
copying a state, mode or `z` field into a cell at stage-1 position (§5.3), and
stage 2 already holds `x`/`m`/`z` directly, so carrying it in the hand-down
would be §7.4 step 4's rejected identity transport relocated from the table
into a bundle.

**Spec.** §5.2, §5.3, §5.6, §7.4, §12.3, §13.2, Appendix B, Appendix D

**Rationale.** It is also what §12.3 already sources (the stage-1 *probe's
...
all, and a stage-2 function destructuring one meets §13.2's framing diagnostic.

Context: this arose while re-examining whether `y_x` survives D-165 at all — it
does, and the theme it was suspected of violating ("`y` is public-only") is
what D-165 delivered: `y_x` is now declared ports only, `w` carries the private
...
- *Dropping `y_x` entirely and re-routing public stage-1 values through `w`
  ...
  `f`/`g`/guards get through `y`, and would unpick §5.6's tracer boundary
...
method*, D-041's endpoint derivation becoming a cross-check with a sharper
...
**Spec.** §3.3, §6.1, §6.2, §9.3, §11.2, §11.5, §11.6, §11.8, §12, §13.3, §16
(swept)

**Rationale.** Supersedes D-041's single-method shape; D-039's marker role
...
**Rationale.** Annotates D-144: its "stays accurate if the output-direction
addition lands" clause assumed a direction *keyword* on one helper; after
D-170's split a single call cannot emit into two declarations, so the guarded
...
§11.8's inputs-only-by-definition paragraph shrinks to the guarded-addition
pointer, the name now carrying the direction.

**Rejected.**
- *Bare `passthrough`:* D-144's premise retired by the split.
- *Plural `input_passthroughs`:* a bare plural collection noun re-enters the
  declaration register D-144 moved the helper out of; return cardinality never
...
**Position.** "Face" is ruled kind-blind and defined at first use (§3.3, §4.3,
§11.2 fixed; the term was never adjudicated — presupposed by D-033/D-041 — this
row records it as deliberate): a component's *ports* are its signal endpoints
(one cell, one producer — §4.3's periphery atom); its *faces* are the names
those ports wear on its boundary — coinciding for a leaf, aliasing interior
ports for an assembly (D-170's declarations), never creating an endpoint; N
...
(§6.1's generic seam requires the kind-blindness).

**Spec.** §3.3, §4.3, §6.1, §9.2, §11.2, §11.3, §11.6

**Rationale.** Wording fixes: §3.3 "exported ports" → boundary faces; §11.2 "a
...
- *Unifying on "port"/"assembly ports":* kind-qualified vocabulary cannot state
  §6.1's generic-seam rule; calling aliases ports falsifies §4.3's
  one-port-one-cell atomicity and forces "port alias" repair vocabulary; "port
  name vs port path" loses the two-notation rule's lexical anchor, §11.6/§9.2.
- *Replacement terms surveyed and rejected:* `gate` (flow-control connotation
  ...
  cells, D-166–D-168, and is the endpoint, not the name), `terminal`
  ...
  the morphological root of §11.3's own "interface".
...
**Spec.** §3.2, §5.2, §5.3, §7.3, §11.2, §11.7, §12.3, §12.5, §13.7, §14.1,
§15.2, §15.5, Appendix C (all companions swept), Appendix D

**Rationale.** D-056 untouched and load-bearing — it is what makes the fusion
...
agreeing, D-166–D-167); a stateless leaf's tier is decided by its contract
...
(walking at `T` vs pinned, D-166–D-167) — and `DeclarationOnWrongTier` survives
...
- *`is_discrete(::C)::Bool` trait as stateless-leaf tier decider:* derivable
  ...
  boilerplate on every leaf, defaulted = class-by-omission, which D-039 refused
  for kind; the corner it protects is rare — §13.7 steers stateless leaves
  continuous — already netted by §11.7's rates-on-continuous error and D-166's
...
**Spec.** §9.3, §9.4, §9.6, §9.7

**Rationale.** A second greedy attach finds an empty complement and stakes an
**empty claim** — legal, §9.6's honest may-write-nothing degenerate — plus the
attach-time diagnostic `EmptyGreedyClaim` (warning (service), D-159's sixth
...
Supersedes the interactive-register halves of D-044, D-104 and D-106.
...
at run exit it detaches what it attached — the detach sitting in §10.4's
...
**Spec.** §9.6, §10.4, §10.6, Appendix B

**Rationale.** Motivation: under D-174's computed claims a persisted greedy
...
**Rejected.**
- *The prior §10.6 semantics — an idempotent flag that never detaches:* hidden
...
D-107's density rule): the verbatim-vs-sparse dichotomy is eliminated —
...
order unchanged and D-029's two-orders-below-the-log budget untouched); the
conversion allocates once per drained batch, in-class with §7.5's retention
...
header or replay. Motivation: D-174 made "enumerated ⇒ claim-narrow ⇒ dense"
false — a greedy claim is enumerated and as wide as the root contract — so
density could only be re-keyed on the claim source, i.e. on a distinction D-174
exists to erase.
...
dispatch — §11.5's one-root rejection does not transfer, the periphery's
single-inheritance slot being vacant (no domain hierarchies compete for it) and
a binding's sidedness being its **public contract**, not the §11.3-hidden
implementation class a component's tier is.

**Spec.** §9.3, §9.6, §11.1, §11.3, §11.5, Appendices A/B/C, glossary D.6
...
(§11.1's reflection class, run at a stopped-sim service point).
...
idiom the framework never calls — taught in Appendix A, never checked. What
this closes: under method-presence detection a bidirectional binding whose
`claims` was written *without* extending the framework generic (the §11.1
...
makes of it — annotated inline where §9.6 lists the rejection. Side-encoding
...
- *Side-encoding supertypes `AbstractInputBinding`/`AbstractOutputBinding`:*
  ...
  subtype two sides.
- *Annotation (from §9.6's inline treatment, migrated at the P3.2 rewrite,
...
idiom — no spec change (D-177's clarity-supertype argument explicitly does
*not* transfer).
...
(`f`-vs-`g` plus the §11.5 signature arities, `TierSignatureMismatch`), which
is D-173's `is_discrete` rejection restated one level up: the periphery's
...
the declaration set (§11.5's class-by-declaration-shape), they silence both
D-164's declares-nothing build error and §11.1's shadowing defense — the two
...
boilerplate-on-every-leaf outcome D-039, D-173 and this row all refuse.

**Rejected.** The rejected alternatives are this row's subject:
- *Tier supertypes* `ContinuousComponent`/`DiscreteComponent`.
- *A nominal `is_discrete`-style tier marker* (D-173, upheld).
...
predicate (`σ ≥ 0`, the §2.1 definition, semantics-preserving by construction).

**Spec.** §2.1, §8.4, §11.1, §12.3, §12.5
...
- *The `localize` keyword, the prior design:* redundant with the probed form —
  the §12.3 probe already observes it; every policy flip touches two sites,
...
- *Per-relation-atom crossing decomposition à la Modelica:* requires owning the
  AST — excluded by §11.1's no-macros/no-introspection foundations.
...
case, and the domain's transitions are asymmetric-handler pairs (D-082:
...
**Rejected.**
- *Building it now:* machinery cheap but no grounded use case; §2.1's
...
budget·events) instead of structural; rescues D-020's rejected rounds-cap in
repaired form (per-event, degrade-not-error — the §8.4 doctrine that postdates
...
- *Honest prior + explicit pending bit:* identical behavior to the lie with one
  more register, and composes worse with D-182's θ=0 check.
- *A joint firing pool:* one chatterer starves innocent cascades — D-020's
...
D-181's honest priors (drain = sole disagreement source).

**Rejected.**
- *Forbidding `u`-reading localized guards:* unenforceable without the
  introspection §11.1 forbids; kills the gate idiom D-179 blesses.
...
- *Frozen-`u` probes:* a sweep has one `u`; two-epoch bundles are §8.6's
...
retired from `run!` (the poison was its only referent); supersedes D-077's
poison clause.

**Rejected.**
- *Scoped eltype poison of record (D-077: `NaN`/`typemin` + loud
...
field contributes path-named children via §11.5's container-children rule and
...
persona with zero new declaration rules — §11.5's builder rejection stands
untouched (immutable grouping needs no builder), and the component enters
§13.7's inventory as its one persona-admitted member.

**Rejected.**
- *The mutable builder (`Assembly()` + `add!`/`connect!`):* §11.5's standing
...
**Spec.** §8.5, §11.7, §12.7, §13.1, §14.5, §14.8
...
first tick at `Φ·Δt_base` (§14.5's sweep bullet and §14.8's committed-residual
...
- *Constructor-side range validation:* instant REPL feedback, but fails a
  `sample_times` body's *evaluation* with a raw exception against §13.1's
...
**Rejected.**
- *Root-only absolute declaration (the superseded §8.5 clause):* right about
...
§14.7's worked example already uses.

**Spec.** §14.7, §14.9

**Rationale.** So the type's own constructor honors the rule §14.7 states for
...
- *Keeping the positional lift:* `lower`/`upper` are adjacent and same-typed,
  silently swappable — precisely the hazard D-074 rejected positional
...
address — the device handle inside the task, the roster entry in §10.4's
pre-spawn init bracket, where no handle exists yet.

**Spec.** §9.6, §9.8, §10.4, Appendix B, Appendix C

**Rationale.** The address supplies the device identity, which is why no call
passes a device id even though Appendix C lists it in both payloads; D-160's
definition and Appendix B's handle-capability listing stand unchanged.

**Rejected.**
- *Renaming the framework-internal form:* treats one operation — write a
  device-attributed diagnostic into that device's §9.8 cell — as two, when only
...
- *Keeping the kind-first positional form `report!(DeviceCrash, dev, e)`:* a
  ...
  site uses, and one name owning two unrelated signatures — the polysemy D-122
  legislates against.
...
**Rationale.** Derived from the closed leaf vocabulary (D-094) and the
build-time flat layout — derivative completeness is a property of the layout,
checked structurally at §12.5 (each field of `f`'s return scatters into its
...
- *A per-leaf `derivative_type` override hook:* a second register for a fact
  ...
  off its own type, is exactly what D-094's closed vocabulary excludes.

### D-191 — Defer, not consume, the edge on a blocked event

**Status.** ratified

**Position.** Blocked-event edge mechanics (completes D-154's serialization;
...
§8.6's unconditional prior update stands; firing counts untouched (they track
...
- *Marking the blocked event for unconditional firing next round:*
  fire-on-falsified-premise reborn — the defect D-154 exists to kill — and the
  D-153/D-181 deferral shape: a firing manufactured from bookkeeping rather
