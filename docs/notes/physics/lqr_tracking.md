...
Here, $z$ represents a vector of output variables to be commanded, rather than the system's complete measurement vector, and our goal is to drive it to a reference value $z^{\ast}$.
...
To achieve $z = z^{\ast}$, we need to drive $\Delta z$ to $\Delta z^{\ast} = z^{\ast} - z_{trim}$ in the linearized system.

Now we seek an equilibrium reference $(\Delta x^{\ast}, \Delta u^{\ast})$ such that:

$$
0 = A \Delta x^{\ast} + B \Delta u^{\ast}
$$

$$
\Delta z^{\ast} = C \Delta x^{\ast} + D \Delta u^{\ast}
$$

In matrix form:

$$
L \begin{pmatrix} \Delta x^{\ast} \\ \Delta u^{\ast} \end{pmatrix} = \begin{pmatrix} 0 \\ \Delta z^{\ast} \end{pmatrix}
$$

$$
\begin{pmatrix} \Delta x^{\ast} \\ \Delta u^{\ast} \end{pmatrix} = M \begin{pmatrix} 0 \\ \Delta z^{\ast} \end{pmatrix}
$$
...
$$
\Delta x^{\ast} = M_{12} \Delta z^{\ast}
$$

$$
\Delta u^{\ast} = M_{22} \Delta z^{\ast}
$$

Subtracting equations:

$$
\Delta \dot{x} = A (\Delta x - \Delta x^{\ast}) + B (\Delta u - \Delta u^{\ast})
$$

$$
\Delta z - \Delta z^{\ast} = C (\Delta x - \Delta x^{\ast}) + D (\Delta u - \Delta u^{\ast})
$$

If we define:

$$
\Delta \tilde{x} \triangleq \Delta x - \Delta x^{\ast}
$$

$$
\Delta \tilde{u} \triangleq \Delta u - \Delta u^{\ast}
$$

$$
\Delta \tilde{z} \triangleq \Delta z - \Delta z^{\ast}
$$
...
$$
\Delta z = C \Delta x^{\ast} + D \Delta u^{\ast} = \Delta z^{\ast}
$$

The optimal control law is implemented as:

$$
\Delta u = \Delta u^{\ast} + \Delta \tilde{u} = \Delta u^{\ast} - K\Delta \tilde{x} = \Delta u^{\ast} - K \Delta
x + K \Delta x^{\ast} = M_{22} \Delta z^{\ast} - K \Delta x + K M_{12} \Delta z^{\ast} = (M_{22} + KM_{12}) \Delta z^{\ast} - K\Delta x
$$

$$
\Delta u = (M_{22} + KM_{12}) \Delta z^{\ast} - K\Delta x = K_{fwd}\Delta z^{\ast} - K_{fbk} \Delta x
$$

$$
u = u_{trim} + K_{fwd}(z^{\ast} - z_{trim}) - K_{fbk} (x - x_{trim})
$$
...
$$
\xi = \xi(0) + \int^{t}_{0} \Delta \tilde{z}_{i} d\tau = \xi(0) + \int_0^{t} (\Delta z_{i} - \Delta z_{i}^{\ast}) d\tau = \xi(0) + \int_0^{t} (z_{i} - z_{i}^{\ast}) d\tau 
$$
...
$$
\dot{\xi} = \Delta \tilde{z}_i =0 = z_i - z_i^{\ast}
$$

Let's suppose we have an external disturbance such that the actual system dynamics are instead:
...
0$, and therefore necessarily $z_i = z_i^{\ast$. Because the system is no longer homogeneous, in general we
...
$$
\Delta u = \Delta u^{\ast} + \Delta \tilde{u}= \Delta u^{\ast} - K_{aug}\Delta \tilde{x}_{aug} = \Delta u^{\ast}
- K_x \Delta \tilde{x} - K_{\xi} \xi = \Delta u^{\ast} - K_x \Delta x + K_x \Delta x^{\ast} - K_{\xi} \xi 
$$

$$
\Delta u = M_{22} \Delta z^{\ast} - K_x \Delta x + K_x M_{12} \Delta z^{\ast} - K_{\xi} \xi= (M_{22} + K_x M_{12}) \Delta z^{\ast} - K_x \Delta x - K_{\xi} \xi
$$

$$
\Delta u = K_{fwd} \Delta z^{\ast} - K_{fbk} \Delta x - K_{\xi} \xi
$$

$$
u = u_{trim} + K_{fwd}(z^{\ast} - z_{trim}) - K_{fbk} (x - x_{trim}) - K_{\xi} \xi
$$
...
$$
\xi = \xi(0) + \int_0^t (z_i - z_i^{\ast}) d\tau
$$

For a practical implementation, we can write:

$$
u = u_{trim} + K_{fwd}(z^{\ast} - z_{trim}) - K_{fbk} (x - x_{trim}) + u_{int}
$$

Where:

$$
u_{int} = -K_{\xi} \xi(0) - \int_{0}^{t} K_{\xi} (z_{i} - z_{i}^{\ast}) d\tau = u_{int}(0)  - \int_0^{t} K_{\xi} (z_{i} - z_{i}^{\ast}) d\tau = u_{int}(0) - \int_{0}^{t} K_{int} (z - z^{\ast}) d\tau
$$
