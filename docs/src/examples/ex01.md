...
is central to the ```Flight.jl``` framework, and we will look into it in future examples.
...
The panel above provides basic simulation controls. You can use it to pause or resume the
simulation and set its pace relative to wall clock time.

The panel below is specific to the simulated ```System```. A complex ```System``` comprises a
hierarchy of components. Each component is itself a ```System```, and it typically defines its own
GUI panel, which can be accessed from that of its parent. Feel free to explore the GUI to get a
picture of the underlying component hierarchy and discover different ways of interacting with the
simulation.

To abort the simulation, simply close the main GUI window.
...
Let's return to X-Plane for a moment. You should now see the aircraft sitting on the runway *with
...
Back in the GUI, open the *Aircraft > Avionics > Flight Control* panel. Expand the *Engine* section
