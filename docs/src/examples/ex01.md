...
customizable sea-level conditions and wind velocity, and ```HorizontalTerrain``` is a terrain model
...
*UPDATE SCREENSHOT WITH FLIGHT DATA EXPANDED*
...
When you are done, enable the *EAS + Climb Rate* mode and set the *Climb Rate* command to zero. This
...
provide automatic turn compensation. For this, you can try one of the higher level lateral modes,
...
observe how the sideslip angle is held close to zero during the maneuver.

Finally, let's move on to the *Course Angle + Sideslip* mode. (TO DO)

Go to wind and add some crosswind. Notice how heading changes, turns into the
wind to maintain the commanded course

Feel free to try the rest of the control modes and the altitude guidance function. For more
information on ```Cessna172Xv1```'s flight control system, you can check out the source code as well
as the design notebooks in (internal LINKS, see Documenter.jl).

One last thing. If the whole start and take-off sequence starts getting tedious, you can speed it up
by increasing the simulation pace. However, you can also skip it altogether by using the ```trim!```
function to initialize the aircraft in flight. Here's how to do it:
