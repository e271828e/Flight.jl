...
get into it here. If you happen to have a T.16000M available, you can plug it in now and do the
following:
...
Otherwise, don't worry; in this example, you can let the SAS and autopilot modes handle the aircraft
for you, so you can easily control it through the GUI.

The last step is to define a suitable initial condition. For on-ground initialization, we can do
that by specifying the aircraft's body frame kinematic state:

```@example ex01
init_gnd = KinInit(;
    loc = loc_LOWS15, #2D location
    h = h_LOWS15 + C172.Δh_to_gnd, #altitude, as an offset with respect to ground elevation
