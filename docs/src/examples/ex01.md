...
First, expand the *Engine* section. Set the *Mixture* slider to around 0.5. Then, press and hold the
*Engine Start* button to start the engine.

Next, expand the *Longitudinal Control* and *Lateral Control* sections. The longitudinal control
channel computes throttle and elevator commands. The lateral control channel computes aileron and
rudder commands. Both are initialized in *Direct* mode, wherein each actuator command is set
directly to the sum of its corresponding *Axis* and *Offset* values. With *Direct* mode active on
both channels, you are essentially flying the base Cessna 172S model, except for the additional
actuator dynamics.

Try moving the control surfaces using the *Axis* and *Offset* inputs, and observe the result on the
GUI's numerical displays or an external X-Plane camera. When you are done, remember to reset all
values back to zero.
...
Under *Lateral Control*, select the *Roll/Yaw SAS* mode, which will also be put on standby. In this
mode, both aileron and rudder inputs are fed to a MIMO lateral compensator, which computes aileron
and rudder actuator commands. This improves the aircraft's Dutch roll response and stabilizes its
naturally unstable spiral mode.
...
the example without X-Plane.

SHOW SCREENSHOT WITH FLIGHT DATA EXPANDED
...
doesn't actually exist!). When you're ready, just set the *Throttle Input* all the way to 1 and let
the aircraft accelerate and lift off on its own.

Once airborne, you will see the selected modes change from amber to green. Let the aircraft climb
for a while to give yourself some ground clearance. Then, spend some time flying with the SAS modes
enabled. When you feel comfortable with the controls, try switching back and forth between the
*Direct* and *Pitch SAS* modes and compare the aircraft's response to elevator input. You can do the
same with the *Direct* and *Roll/Yaw SAS* modes, but be careful with that unstable spiral mode!

Next, switch to *EAS + Pitch Angle* mode. This one combines an autothrottle loop that tracks
equivalent airspeed with a pitch angle loop built around the pitch SAS. Activating this mode will
disable all *Throttle* and *Elevator* inputs and enable the *Equivalent Airspeed* and *Pitch Angle*
sliders. The numerical readout to the right of each slider shows the current value of the its target
variable. In most cases, when a new control mode is engaged, its target variables will be
automatically set to their current values to ensure a smooth transition.

When you are done experimenting, feel free to try the rest of the higher level longitudinal modes.
If you crash, just reset the simulation and have another go:
```julia
Sim.init!(sim, init_gnd)
Sim.run_interactive!(sim)
```

You may have noticed that the *Roll/Yaw SAS* mode does not provide automatic turn compensation. For
this, we need to enable one of the higher level lateral modes, all of which provide sideslip angle
tracking. For example, let's try the *Bank Angle + AoS* mode.

For more information on ```Cessna172Xv1```'s flight control system, you can check out the source
code and the notebooks in (internal LINKS, see Documenter.jl), which document the design process
from scratch.
