...
The first panel provides basic simulation controls. You can use it to pause or resume the
simulation, and set its pace relative to wall clock time. To abort the simulation, simply close the
main GUI window.

The second panel is specific to the simulated ```System```. A complex ```System``` like ours
...
Now return to the GUI and navigate to the *Aircraft > Avionics > Flight Control* panel. Expand the
*Engine* section. Set the *Mixture* slider to around 0.5. Then, press and hold the *Engine Start*
button to start the engine.

Next, expand the *Longitudinal Control* and *Lateral Control* sections. Under *Longitudinal
Control*, select the *Throttle + Pitch SAS* control mode. Under *Lateral Control*, select the *SAS*
lateral control mode. The buttons will change to amber, indicating the modes are preselected but not
active. They will engage automatically upon lift off.

![Flight Control GUI](gui_flight_control.png)

That is all we really need to set up for take off. We will not bother with minutiae like keeping
the aircraft centered on the runway or rotating. When you're ready, just move the *Throttle Input*
slider all the way to the right and let the aircraft go.

Once in the air, the SAS modes will keep everything under control. You can experiment with the
primary control sliders (throttle, elevator, aileron, rudder) sliders, or.

Note: If you have a supported joystick attached, the throttle and control surface sliders in the GUI
will be overridden by the corresponding joystick axes.

different autopilot modes. Here is a brief description of what's available:

Longitudinal:

Throttle + Pitch SAS: Combines direct throttle control with a pitch stability augmentation loop that
tames the short-period and phugoid modes. Active inputs: *Throttle Input* and *Elevator Input*
...
Brief description of autopilot modes and altitude guidance. Which commands are active in
each case.




!!! tip "Keyboard Input for Sliders"

    To assign a precise value to a control slider, use Ctrl+Click (on Windows)
    or Cmd+click (on Mac) to enable keyboard input.


Notes: To avoid transients, when a new autopilot mode is engaged, its command variables will be
automatically set to their current values.
