...
function client_callback(sim_out::Sim.Output)
    freq = 0.5
...
function server_callback!(sys::System{PID}, str::String, ::InputMapping)
    JSON3.read!(str, sys.u)
end
...
function json_sketch1()
    ...
    Sim.attach!(sim, UDPClient(; port = 49017, callback = client_callback))
    Sim.attach!(sim, UDPServer(; port = 49017, callback = server_callback!))
    Sim.run_paced!(sim)
