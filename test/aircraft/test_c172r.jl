...
function test_trimming()
    ...
    @testset verbose = true "θ Constraint" begin
        ...
        @test @ballocated(C172R.θ_constraint(; v_wOb_b = $v_wOb_b, γ_wOb_n = $γ_wOb_n, φ_nb = $φ_nb)) === 0
    ...
    @testset verbose = true "Assignment" begin
        ...
        C172R.assign!(ac, env, params, state)
        ...
        @test (@ballocated C172R.assign!($ac, $env, $params, $state))===0
    ...
    @testset verbose = true "Optimization" begin
        ...
        f_target = C172R.get_target_function(ac, env, params)

        @test @ballocated($f_target($state)) === 0

        exit_flag, _ = C172R.trim!(ac, env, params, state)
