...
export test_c172rpa1


function test_c172rpa1()
    @testset verbose = true "Cessna172X1" begin
...
y_kin(ac::System{<:Cessna172RPA1}) = ac.y.vehicle.kinematics
y_air(ac::System{<:Cessna172RPA1}) = ac.y.vehicle.air
y_aero(ac::System{<:Cessna172RPA1}) = ac.y.vehicle.components.aero
...
function test_control_modes()
    ...
    ac = Cessna172RPA1(WA(), trn) |> System;
    ctl = ac.avionics.ctl
...
function Systems.extract_output(sys::System{<:Cessna172RPA1},
                        ::UDPOutput,
                        ::JSONTestMapping)
    freq = 0.1
    φ_sp_max = π/6
    ...
    if sys.t[] > 5
        ...
        #methods defined in C172RPA.C172RPAControl
        cmd = (
...
function Systems.assign_input!(sys::System{<:Cessna172RPA1},
                                data::String,
                                ::JSONTestMapping)
    #ControllerU is declared as StructTypes.Mutable() in C172RPA.C172RPAControl,
...
function test_sim(; save::Bool = true)
    ...
    kin_plots = make_plots(TimeSeries(sim).vehicle.kinematics; Plotting.defaults...)
    air_plots = make_plots(TimeSeries(sim).vehicle.air; Plotting.defaults...)
    save && save_plots(kin_plots, save_folder = joinpath("tmp", "test_c172rpa1", "sim", "kin"))
    save && save_plots(air_plots, save_folder = joinpath("tmp", "test_c172rpa1", "sim", "air"))
