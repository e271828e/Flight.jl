...
function test_strut()
    ...
    @testset verbose = true "Strut" begin
        ...
        #wow = false
        kin = KinematicInit( Ob = GeographicLocation(l2d, h_trn + 1.1)) |> KinematicData
        ...
        kin = KinematicInit(; Ob, v_eOb_n = [10,0,1]) |> KinematicData
        f_cont!(strut, steering, terrain, kin)
        @test isapprox.(strut.y.v_eOc_c, [10, 0, 0], atol = 1e-5) |> all

        #lateral motion and compression
        kin = KinematicInit(; Ob, ω_lb_b = [1,0,0]) |> KinematicData
        f_cont!(strut, steering, terrain, kin)
...
function test_contact()
    ...
    @testset verbose = true "Contact" begin
        ...
        kin = KinematicInit(; Ob, v_eOb_n = [1e-4, 0, 0] ) |> KinematicData
        f_cont!(strut, steering, terrain, kin)
        f_cont!(contact, strut, braking)
        ...
        #braking
        kin = KinematicInit(; Ob, q_nb = REuler(φ = 0), v_eOb_n = [1e-4, 0, 0] ) |> KinematicData
        braking.u[] = 1
        f_cont!(braking)
        f_cont!(strut, steering, terrain, kin)
        f_cont!(contact, strut, braking)
        ...
        kin = KinematicInit(; Ob, q_nb = REuler(φ = π/12), v_eOb_n = [1e-4, 0, 0] ) |> KinematicData
        f_cont!(strut, steering, terrain, kin)
        f_cont!(contact, strut, braking)
        @test contact.y.wr_b.F[2] < 0

        #lateral motion, small velocity
        kin = KinematicInit(; Ob, q_nb = REuler(φ = 0), v_eOb_n = [0, -1e-4, 0] ) |> KinematicData
        f_cont!(strut, steering, terrain, kin)
        f_cont!(contact, strut, braking)
        @test contact.y.wr_b.F[2] > 0
        ...
        kin = KinematicInit(; Ob, q_nb = REuler(φ = 0), v_eOb_n = [0, -1, 0] ) |> KinematicData
        f_cont!(strut, steering, terrain, kin)
        f_cont!(contact, strut, braking)
        ...
        #wow = false
        kin_data = KinematicInit( Ob = GeographicLocation(l2d, h_trn + 1.1)) |> KinematicData
...
function test_landing_gear_unit()
    ...
    @testset verbose = true "LandingGearUnit" begin
        ...
        #wow = true
        kin = KinematicInit( Ob = GeographicLocation(l2d, h_trn + 0.9), v_eOb_n = [0,1,0]) |> KinematicData
