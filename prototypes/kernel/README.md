...
Increment 21 is under way, and its first two stages are here: the compile
...
services that will instantiate their own — and §14.4's read side stands beside
the write side the condition algebra already had: the closed read-selector
family, the compiled reader that is `apply!`'s gather twin over an executor,
and `capture`, which reads the committed world back as a total condition.
...
- **§14 beyond increments 18 and 21's slices** (the condition algebra,
  resolution, totality and `init!`; the read side and `capture`): the
  *specialized* `apply!` register with its `Getter{P}` lenses, per-shape
  codegen and folded shape check (§14.3, §14.4, D-066) is absent with trim,
  the iterating service it exists for — the dynamic walk is the only register
  here, and the plan records no tree positions at all, the baked destination
  standing in their place; so are the non-nominal `Float64 → Dual` converter
  arm of §14.3's table, `trim!`, `linearize` and §14.9's mounting (`at` over a
  problem or a read set: a read set is a flat labeled value here, relocated by
  nothing);
